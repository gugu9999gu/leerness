...
- Test evidence ID: T-0174 (released in v1.36.186; no runtime migration, common store, finalize, MCP or provider execution in this stage).

## Runtime compatibility v1 — P-0021 approved, legacy layout retained
- Feature: `state compatibility [path] [--path path] [--json]` and the shared runtime writer operation diagnose/refuse observed incompatible layouts without moving data. The explicit path flag takes precedence for diagnosis; writer admission uses each existing handler's actual target and precedence.
- Input: The fixed `<GIT_DIR>/leerness/layout.json` for a whole Git worktree, or canonical `.leerness/cache/state-layout.json` for a genuine non-Git project. The exact six scalar fields, 16KiB limit, UTF-8, regular-file identity and safe parents are validated without repair. Unknown/duplicate fields, future schema/layout/protocol, alternate/new runtime indicators, unsafe links, ambiguous ownership and unresolved topology/backend changes block writes.
- Output: Bounded `leerness.runtime-compatibility/v1` JSON with observed layout, scope, reason, write disposition, `activationSupported:false` and `workspaceAdmission:operation-start`. Diagnosis bypasses startup migration, usage, presence, skill-catalog and update bookkeeping; existing metadata-only `state inspect` remains separate.
- States: Descriptor absent or strictly supported legacy descriptor permits observed legacy writes only when no conflicting indicator exists. Workspace ownership/selection is admitted once per operation; each actual mutation rechecks fresh descriptor/indicator and path identity. New CLI/MCP/REPL calls re-admit. Exact known-store producer lock/release/temp/create names preserve normal concurrency; unknown names and links do not become ownership evidence.
- Errors: Writer admission precedes CLI bookkeeping. Direct operations latch `E_RUNTIME_LAYOUT_INCOMPATIBLE` so best-effort catches, retained FDs used from another operation and MCP telemetry cannot turn rejection into success; CLI renders `runtime_layout_incompatible`. Existing workspace selector error codes and direct store read diagnostics are preserved. Rechecks follow lock waiting, heartbeat ticks, each Windows replacement subprocess attempt and mutating Git calls.
- Limits: No descriptor creation, data copying, activation, control store, finalize or cleanup. This is not atomic fencing and does not stop old installed clients, external writers or a change after the final observation. Arbitrary project-file writers need explicit root scope; already prepared recovery material is retained on late rejection.
- Optimization: One immutable Git topology/workspace ownership snapshot per operation; fresh compatibility, bounded fixed-path metadata, no recursive store-content scan. A measured 50-read fixture uses one Git query and one workspace enumeration, not a claimed throughput increase.
- Related files: `lib/runtime-layout.js`, `lib/runtime-writes.js`, `lib/io.js`, `lib/git.js`, `bin/leerness.js`, `docs/runtime-compatibility-api.md`, `docs/worktree-runtime-migration.md`, `scripts/runtime-layout-probe.js`, `scripts/runtime-write-probe.js`, `scripts/runtime-git-write-probe.js`, `scripts/runtime-replacement-probe.js`.
- Test evidence ID: T-0180 (implementation present; full regression, supported CI and publication pending).

## Encoding selftest failure diagnostics — T-0181
- Feature: The UR-0113 encoding safety selftest retains bounded failed replacement details and restores the caller's prior `process.exitCode`, including an unset value.
- Input: Actual canonical and junction-backed Windows replacements, plus deterministic failed-replacement fault injection with oversized diagnostic text and prior exit unset/23.
- Output: Failure reports `UR-0113` with bounded scan/apply error and throw/exit context rather than discarding the captured JSON. Existing byte-exact encoding assertions remain unchanged.
