...
- v1.36.183 후보에서 English `skill list`의 4줄 누수를 stored/env/explicit locale 모두 0으로 만들고 39-command 측정 래칫을 34→30으로 강화함. 한국어 유효 메타데이터 바이트와 canonical JSON 값은 보존됨
- 전체 `npm test` 467/467(5,068초), 지연 경쟁 11/11, 범프 후 `test:fast`와 smoke 13/13 통과. 외부 Codex 최종 재검토는 actionable P0/P1/P2 없음
- v1.36.183 GitHub/npm/leerness.com 공개 및 CI 13/13 검증 대기
...
- v1.36.183 공개·CI 검증 후 T-0092를 정확한 30줄/16명령에서 계속 낮춤. 다음 최대 클러스터는 mode·roles list·permissions list·session-resume 각 3줄
