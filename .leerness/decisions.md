...
- Impact: agents quota schema v2가 설치·활성·인증·routing·model callability·quota를 분리한다. 비활성 provider는 presence-only이고, credential-like env는 probe 전에 제거된다. 기존 quota 필드는 호환을 위해 null/not-observed로 유지한다.
