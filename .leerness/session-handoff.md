...
Last generated: 2026-09-05T12:08:38.144Z
...
## Verification
```
Task: T-0174 / P-0020 approved / UR-0098; broader UR-0097 remains in-progress.
Command: `npm run test:state-scopes`; Node18 scope and CLI probes; `npm run lint`; command-flags/i18n-next-cluster/API contract checks; independent `codex exec --ephemeral --sandbox read-only` review and controlled fixture reproductions.
Exit: 0 for the completed targeted checks. Full `npm test`, focused Codex re-review and publication are still pending at this entry.
Tests: 104/104 targeted checks passed (workspace selector26, scope58, CLI20); Node18 scope58/58 and CLI20/20 passed. Lint: 84 JS + 1 JSON clean. Existing command-flags and seven-surface i18n controls passed after the final CLI locale fix. Site candidate measured version1.36.186, MCP98/core20/deps0/selftest355, unmeasured0 and built710 pages; this is not a deployment claim.
Review: Initial independent Codex returned no P0/P1 and three P2 findings. (1) Quoted path punctuation was misclassified as publish/network; the new pure assertion failed before a narrow quote-aware literal exemption was added. Composite command strings remain pure test data and are never executed. (2) Invalid inspect options read cwd manifest for error language; the real-CLI preload measured projectReads1 before the exact route's flag/env-only locale fix, then0 for all four invalid-input classes. (3) Git-returned missing/unreadable paths and marker-walk stats produced path_not_found/path_unreadable/rawEACCES; the four new checks failed54/58 before Git-specific normalization and passed58/58 afterward. The independent reviewer could not execute Git from Node in its sandbox and did not run fixture-writing probes; those behavioral checks were performed by the main implementer on Windows Node26 and18.
Optimization evidence: one bounded Git topology query, one workspace selection snapshot, cached parent metadata, no recursive inventory, no npm/provider invocation and no project content reads or writes through the real inspection entry point. Target/cwd/Git bytes and mtimes remain unchanged. Both existing canonical and legacy locations are inventoried without merging contents. All proposed scopes stay inactive; existing runtime writers and data locations are unchanged. A review verdict is not substituted for the pending full regression/public release evidence.
Artifacts: lib/state-git.js, lib/state-paths.js, lib/state-inventory.js, lib/state-inspect.js, lib/workspace-dir.js, lib/pure-utils.js, bin/leerness.js, scripts/workspace-selection-probe.js, scripts/state-scopes-probe.js, scripts/state-inspect-cli-probe.js, docs/state-paths-api.md, docs/state-scopes.md.

Independent follow-up verdict: `CLEAN for this delta.` All three initial P2s were re-reviewed as resolved; node --check and a non-mutating classifier smoke passed. The focused reviewer did not execute fixture-writing probes or claim coverage of the original full feature. R-0001 is satisfied by the original full review, independent reproductions/fixes, and this scoped re-review. Full local E2E and the OS/Node CI matrix remain pending. Product source is frozen while those run.
Code/contract lens: reuse the existing sanitized Git gateway and workspace selector, keep the new concerns in four small domain modules, expose inactive proposals honestly, and keep semantic no-write/compatibility probes alongside structural contract verification. No speculative StateManager facade, database, dependency, dispatch adapter or automatic migration was added.

CI fixture correction: candidate6252301 workflow33959389837 passed scope checks on Ubuntu Node18/20/22, but Windows fast failed51/58 because hosted TEMP used RUNNER~1 while the product correctly returned runneradmin. A local junction-backed TMP/TEMP reproduced exactly the same seven assertion/injection misses (51/58). Canonicalizing only each probe's temporary root, including its cleanup parent guard, restored scope58/58 and CLI20/20 under that alias. No assertion or product source changed. Independent read-only follow-up on the two test diffs returned FIXED and confirmed unchanged assertions plus mkdtemp-bound cleanup. The superseded CI run is replaced; its failure is not represented as a successful full matrix.

Full local regression: `npm test` exited0 on 2026-09-05T11:02Z, with E2E467/467 in5272seconds, delayed race pairs11/11 and runtime DEP0190 warnings0. Earlier parts of that long-running command preceded the three final P2 fixes; post-fix targeted104/104, Node18 scope58/CLI20, command-flags/i18n, installed cleanroom10/10 and final tarball scope58/CLI20 were rerun separately. Frozen candidatebd33683 CI33959679140 has passed all seven state-scope platform steps, both fast jobs, all four release-runtime jobs and Ubuntu Node18/20/22 full E2E; four Windows full E2E jobs are still pending at this entry. Do not confuse this intermediate status with a completed13/13 matrix.
Final pack candidate: 97files,2205607bytes,unpacked7238495bytes; SHA1 `2b50abb48c501ee03569017f06d876fa0ffa49bf`, SHA256 `534e8270bd13647b4b41be64fee098aceb30f6fcb6ed51c0c8406ca42dbeace3`, integrity `sha512-mytfzVF3e6QQ5zs/NCLmHkiDK2sP9+nlbyfIQRPW9jkQjusK1HsYGZagV2VBUNkH9dq3N2x1kETk+8i5H6xgVQ==`. All nine new state docs/modules/probes are present; no .env, .git, .leerness or node_modules entry is packed. A separate consumer installed this exact tarball and passed scope58/58 and CLI20/20. This pack is not yet a public registry artifact.

## 2026-09-05 — v1.36.186 frozen CI and public publication

Task: T-0174 / P-0020 / UR-0098. This entry supersedes the pending CI/publication status above, not the documented limits of the earlier local test run.
...
- Independent Codex original read-only review found three P2s; each was reproduced/fixed and focused re-review returned CLEAN. The dispatch.prepared rows below are preparation events, not proof of provider execution or independent review completion; use review-evidence.md for the actual review evidence.
...
- Follow the user's UR-0097 State scope architecture. T-0173 audit/planning and T-0174/M-0014 inspection foundation are complete. M-0015..M-0018 remain planned. M-0010 Role v2 migration must share these contracts. T-0092 remains a separate i18n backlog, not this round's next task.

## Next Exact Step
- T-0175 / M-0015: inventory legacy runtime readers/writers and session ownership, design explicit migration with source preservation, quiescence and interruption recovery, then present its preview for user approval. Do not implement migration or activate a new runtime before that approval.

## Current Round Boundary
- T-0174 done, UR-0098 completed, M-0014 completed100%; broader UR-0097 remains in-progress. v1.36.186 is public on GitHub/npm/leerness.com with matching artifact digests and successful public installation checks.
- Existing Git/workspace boundaries were reused in four narrow state modules. Inspection uses one Git topology query, a shared workspace snapshot and fixed metadata inventory; no content reads, recursive inventory or state writes. Existing data locations and writers remain authoritative. No migration, ControlStore, finalize or new UI was implemented.
- Dedicated104/104, frozen pre-publication CI33959679140 13/13 (full E2E467/467), public selftest355/355 and state inspect PASS. Main push replay33964861084 is a separate automatic run on the identical tested SHA; it is still running, not a second completed matrix.
- Session close exited0 and final gate passed6/6 with149 completed claims,68 historical baseline entries and0 new failures. R-0001: independent Codex review/reproduction/re-review satisfied. R-0002: publication and public verification satisfied; session close's automatic version check also passed185→186.
- Pre-wake critical1 is the old unmatched UR-0028, confirmed by requests audit, not an unhandled UR-0097/UR-0098 or a release failure. The user's dirty saved main checkout remains untouched. No worktree removal or scheduler/video setting change was performed.
