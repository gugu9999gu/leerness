...
function featureAddCmd(root, title, deps = {}) {
  ...
  // 1.36.2 (clean-room, UR-0184): 미초기화 dir 에 stray .leerness scaffold 대신 에러 — task add 와 동일 게이트(_requireInit, --force 우회).
  //   기존엔 _ensureFeatureGraph 가 무조건 .leerness/feature-graph.md 생성 → 비-프로젝트 폴더 조용히 오염. (_requireInit 미주입 시 fail-open: 구버전/직접호출 호환)
