// lib/state-integrity.js — .leerness/*.json 상태파일 JSON 무결성 검사 (단일출처, 1.36.1).
//   배경(클린룸 리뷰 FN): health/doctor/check/audit 는 상태 JSON 을 try{JSON.parse}catch{빈 상태} 로 그레이스풀
//   폴백하느라, .leerness/manifest.json 등이 깨진 JSON 이어도 전부 "healthy"/exit 0 을 반환하던 false-negative 가 있었다.
//   → 어떤 명령도 워크스페이스 상태파일의 JSON 무결성을 검증하지 않던 갭을 이 헬퍼로 메운다(audit/health/check 공유).
'use strict';
const fs = require('fs');
const path = require('path');

// .leerness 바로 아래의 *.json 상태파일을 열거해 JSON.parse 를 시도, 파싱 실패 파일 목록을 반환.
//   반환: [{ file: '.leerness/<name>.json', error: '<message>' }, …]  (파일명 오름차순 — 결정적/테스트 안정).
//   설계:
...
//     - 비-크래시: .leerness 부재/디렉토리 읽기 오류 → 빈 배열, 개별 파일 읽기 오류 → skip.
function findCorruptedStateJson(root) {
  const dir = path.join(root, '.leerness');
  let entries;
  try { entries = fs.readdirSync(dir, { withFileTypes: true }); }
  catch { return []; }  // .leerness 없으면 검사 대상 0 (미초기화는 별도 체크가 담당)
  const corrupted = [];
  for (const e of entries) {
    ...
    try { JSON.parse(text); }
    catch (err) { corrupted.push({ file: '.leerness/' + e.name, error: String((err && err.message) || err).slice(0, 200) }); }
  }
