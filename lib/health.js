...
const { _parseArchiveBlocks, _milestoneSurface, _recentChangesAggregate } = require('./pure-utils');
...
function healthCmd(root, deps = {}) {
  ...
  try {
    ...
    // 1.36.104 (P-0014): 인정된 픽스처 제외 — 네 표면(headline/health/audit/session-close)이 같은 술어를 쓴다.
    const committedSecrets = (sec.unacknowledged || sec.committed).length;
  ...
  // 1.36.1 (클린룸 리뷰 FN): .leerness/*.json 상태파일 JSON 무결성 — 손상 JSON 을 그레이스풀 폴백이 healthy 로 위조하던 false-negative 를 degraded 신호로 표면화.
  try {
  ...
  try {
    ...
    out.memorySurface = {
      ...
      archive: (() => {
        ...
        try {
          const hdHe = path.join(root, '.leerness');
  ...
  try {
    ...
    out.milestones = _milestoneSurface(ms);
  } catch (err) {
    out.milestones = _milestoneSurface({ gitHistoryState: 'unavailable', gitHistoryError: { code: 'git_history_unavailable', reason: 'aggregate_exception', error: String(err && err.message || err) } });
  }
  // 1.9.234: health --json recentChanges 통합 (3 명령 8 필드 일관성)
  try {
    Object.assign(out, _recentChangesAggregate(_computeRecentChanges(root, 5)));
  } catch (err) {
    Object.assign(out, _recentChangesAggregate({ state: 'unavailable', error: { code: 'git_history_unavailable', reason: 'aggregate_exception', error: String(err && err.message || err) } }));
  }
  ...
  try {
    ...
    cap.mcpTools = toolCount >= 50
      ? { score: 100, status: '✓', evidence: t(`${toolCount}/50+ 도구`, `${toolCount}/50+ tools`) }
  ...
  if (out.envInfo?.encodingScanErrorCount > 0) issues.push(t(`인코딩 스캔 읽기 실패 ${out.envInfo.encodingScanErrorCount}건 — 결과 불완전`, `${out.envInfo.encodingScanErrorCount} encoding scan read error(s) — result incomplete`));
  out.issues = issues;
  ...
  else log(t(`  ✓ .leerness/*.json 파싱 정상`, `  ✓ .leerness/*.json parse OK`));
  ...
  if (out.capabilityMatrix && !out.capabilityMatrix.error) {
    ...
    // 1.36.5 (정직성 감사): 이 점수/판정은 소스 존재·의존성 휴리스틱 자가평가일 뿐 — 측정된/독립 성숙도 검증이 아님을 명시(과장 방지).
    log(t(`  ⚠ 소스 존재·의존성 기반 휴리스틱 자가평가 — 측정된/독립 성숙도 판정 아님(미설치 의존성도 부분점수).`, `  ⚠ heuristic self-assessment from source-presence + dependency checks — NOT a measured or independent maturity verdict (a missing dependency still scores partial).`));
