...
//     · `GIT_CEILING_DIRECTORIES` — 탐색 상한을 바꿔 **정상 저장소의 하위 경로를 not-a-repo 로 오판**시킨다(실측).
//     · `GIT_CONFIG_COUNT` / `GIT_CONFIG_KEY_n` / `GIT_CONFIG_VALUE_n` / `GIT_CONFIG` — **한 번의 실행에만** 사는
//       설정 주입이다. 이걸로 `core.hooksPath` 를 주면 우리가 훅을 *다른 저장소* 디렉토리에 쓰고,
//       그 환경변수가 사라진 뒤에는 지목한 저장소에 강제가 **아예 없다**(실측: foreign=True, target=False).
//       설치는 지속되는 변경이므로 한 번짜리 주입 위에 세우지 않는다.
...
const GIT_LOCATION_ENV = ['GIT_DIR', 'GIT_WORK_TREE', 'GIT_INDEX_FILE', 'GIT_COMMON_DIR',
  ...
  'GIT_CEILING_DIRECTORIES', 'GIT_CONFIG', 'GIT_CONFIG_COUNT'];
