...
function _validateLegacyProjection(value, path, problems) {
  ...
  _unknownFields(value, ['file', 'roleKey', 'primary'], path, problems);
...
function validateAgentInstances(doc) {
  ...
  doc.agents.forEach((agent, index) => {
    ...
    if (!_validNullableText(agent.model, { min: 1, max: 200 })) problems.push(_problem('invalid-agent-model', `${agentPath}.model`));
...
function validateLegacyRoleStore(doc) {
  ...
  if (versionPresent && doc.schemaVersion !== LEGACY_SCHEMA_VERSION) {
    problems.push(_problem(
      Number.isInteger(doc.schemaVersion) ? 'unsupported-schema-version' : 'invalid-schema-version',
      `${path}.schemaVersion`,
      doc.schemaVersion,
  ...
  if (roles === undefined) {
    roles = {};
    warnings.push('roles-missing-treated-as-empty');
  ...
  for (const roleKey of Object.keys(roles).sort()) {
    ...
    if (typeof entry.provider !== 'string' || !PROVIDER_ID_RE.test(entry.provider)) problems.push(_problem('invalid-legacy-provider', `${rolePath}.provider`, entry.provider));
...
function projectAgentInstancesToLegacy(agentInstances, baseLegacyDocument) {
  ...
  for (const agent of projected) {
    ...
    const next = { ...current, provider: agent.provider };
