...
function isValidKey(s) { return typeof s === 'string' && KEY_RE.test(s); }
...
function deriveSessionKey(env) {
  const e = env || {};
  if (e.CLAUDE_CODE_CHILD_SESSION === '1') return null;      // sub-agent 는 등록하지 않는다
  const cand = e.CLAUDE_CODE_HOST_SESSION_ID || e.CLAUDE_CODE_SESSION_ID || null;
  return isValidKey(cand) ? cand : null;
}
...
module.exports = {
  SESSIONS_KEEP, SESSIONS_CLOSED_TTL_MS, KEY_RE, FILE_RE, FORBIDDEN_SIGNALS,
