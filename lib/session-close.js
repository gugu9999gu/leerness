...
function sessionClose(root, opts = {}, deps = {}) {
  ...
  try {
    let _pend = [];
    try { _pend = deps.pendingPreviews ? deps.pendingPreviews(root) : require('./clarify').pendingPreviews(root); } catch {}
    jsonResult.pendingPreviews = { count: _pend.length, items: _pend.slice(0, 10).map(p => ({ id: p.id, title: p.title, status: p.status })) };
    if (_pend.length) {
  ...
  } catch {}  // 1.9.57: --suggest 옵션 — 마감 시 skill suggest + drift check + lessons 통합 보고
