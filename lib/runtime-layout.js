...
const STANDALONE_STORE_FILES = new Set([
  ...
  // Existing init-free CLI outputs; individual command prerequisites remain
  // enforced by their domains. See runtime-compatibility-api.md for producers.
...
// Exact output directories of existing init-free producers: preview builders,
// API/ordinary skills, personas, review prompts, incident storage and skill
// publish staging. Admission checks only their kind, not contents or domain
// validity. Recognizing a staging directory never authorizes a publication.
const STANDALONE_STORE_DIRECTORIES = new Set([
...
function standaloneEntryAllowed(directory, name) {
  const knownDirectory = STANDALONE_STORE_DIRECTORIES.has(name);
  ...
  if (!STANDALONE_STORE_FILES.has(name) && !knownDirectory && !recovery && !knownAuxiliary && !STORE_CREATE_FILE.test(name)) return false;
  ...
  return knownDirectory || (knownAuxiliary && auxiliary[2].startsWith('lock')) ? stat.isDirectory() : stat.isFile();
}
...
// Internal writer identity: existing aliases and Windows namespaced paths must
// share one operation admission. Preserve the reader's bounded failure taxonomy.
function canonicalRuntimeRoot(root) {
  try { return projectLocation(root).root; }
  catch (error) {
    throw Object.assign(new Error('Runtime project identity could not be accepted.'), {
      code: 'E_RUNTIME_LAYOUT_INCOMPATIBLE', reasonCode: failureReport(error, 'unknown').reasonCode,
    });
  }
}
// Only new writer operations join an existing legacy migration. Waiting is
// read-only: lock presence never grants ownership or admits a conflicting
// workspace. Public diagnosis above remains immediate and unchanged.
function createRuntimeWriterAdmission(root = process.cwd(), lockWaitMs) {
  let waitedForLock = false;
  const clock = () => Number(process.hrtime.bigint() / 1000000n);
  const started = clock();
  const result = reader => ({ reader, waitedForLock, lockWaitedMs: waitedForLock ? clock() - started : 0 });
  try {
    const location = projectLocation(root);
    const lock = path.join(location.root, '.leerness-workspace-migration.lock');
    const configured = lockWaitMs === undefined ? envValue('LEERNESS_WORKSPACE_MIGRATION_LOCK_WAIT_MS') : lockWaitMs;
    const parsed = configured === '' || configured === undefined ? NaN : Number(configured);
    const budget = Number.isFinite(parsed) ? Math.max(0, Math.min(60000, parsed)) : 10000;
    const deadline = started + budget;
    const contention = () => {
      waitedForLock = true;
      if (clock() >= deadline) fail('workspace_dir_migration_locked');
    };
    const pending = () => {
      const stat = statMaybe(lock);
      if (!stat) return false;
      if (!stat.isFile() || stat.isSymbolicLink() || stat.nlink !== 1n) fail('workspace_dir_migration_locked');
      return true;
    };
    ...
    for (;;) {
      while (pending()) {
        contention();
        Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, Math.max(0, Math.min(25, deadline - clock())));
      }
      const reader = createRuntimeCompatibilityReader(location.root);
      const report = reader();
      const workspaceConflict = ['workspace_dir_conflict', 'workspace_ambiguous'].includes(report.reasonCode);
      // Once observed, an incompatible descriptor/topology is terminal for
      // this operation, even if another process removes it before check().
      if (!report.compatible && !workspaceConflict) return result(() => report);
      // A peer can start while the snapshot is being classified. Only after
      // retaining any observed non-workspace denial may we join and refresh.
      if (pending()) { contention(); continue; }
      // A peer may also finish AND release between those observations. One
      // fresh classification closes that stale-conflict path without turning
      // genuine persistent conflicts into a retry loop or permission grant.
      if (!refreshedConflict && workspaceConflict) {
        refreshedConflict = true;
        continue;
      }
      return result(reader);
    }
  } catch (error) { return result(() => failureReport(error, 'unknown')); }
}
function assertRuntimeWriteAllowed(root) {
  const report = createRuntimeWriterAdmission(root).reader();
...
module.exports = { MAX_DESCRIPTOR_BYTES, createRuntimeCompatibilityReader, inspectRuntimeCompatibility,
  assertRuntimeWriteAllowed, createRuntimeWriterAdmission, canonicalRuntimeRoot };
