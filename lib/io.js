...
function log(s = '') {
  ...
  if (_failLatched || _jsonErrEmitted) {
    ...
    if (t && (t[0] === '{' || t[0] === '[')) { try { JSON.parse(t); console.log(s); _stdoutWrote = true; return; } catch {} }
...
function fail(s) {
  ...
  if (_argvJsonMode()) {
    ...
    if (!_exitHooked) {
      _exitHooked = true;
      process.on('exit', () => { if (_failLatched && !_stdoutWrote && _firstFail != null) console.log(JSON.stringify({ ok: false, error: _firstFail, code: 'error' }, null, 2)); });
    }
