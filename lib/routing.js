...
const { assessReviewerIndependence } = require('./role-fallback');
...
function plan(root, cls, deps = {}) {
  ...
  const assignments = spec.roles.map(role => {
    let r = null;
    try { r = resolve(root, role); }
    catch (error) {
      if (!roleStoreFailure) roleStoreFailure = { code: (error && error.code) || 'role_store_error', message: (error && error.message) || 'agent-roles.json을 읽을 수 없습니다' };
    }
    const provider = r && typeof r.provider === 'string' ? r.provider.trim() : '';
    return { role, provider: provider || null, model: (r && r.model) || null, modelFamily: (r && r.modelFamily) || null, resolved: !!provider };
  });
  ...
  const reviewerAssessment = !reviewer || !reviewer.resolved || !coder || !coder.resolved
