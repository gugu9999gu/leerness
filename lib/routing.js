...
const _AVAIL_NOTE = '역할별 인증 표시는 무료·비대화형 확인 명령이 있는 provider 만 판정한 결과입니다(나머지는 "확인안됨") — 모델 사용권한·쿼터·과금 권한은 어느 provider 도 확인하지 않았습니다(상세: leerness agents check).';
...
const _EXEC_NOTE = 'leerness 는 여기서 작업을 대신 수행하지 않습니다 — 계획을 확정하고 기록할 뿐이며, 모델 호출(과금)은 하지 않습니다. (고위험 등급에서는 인증 확인을 위해 해당 CLI 의 상태 명령만 실행합니다.)';
...
function plan(root, cls, deps = {}) {
  ...
  const assignments = spec.roles.map(role => {
    const r = resolve(root, role);
    const provider = r && typeof r.provider === 'string' ? r.provider.trim() : '';
    return { role, provider: provider || null, model: (r && r.model) || null, resolved: !!provider };
  });
  ...
  // 독립 검수: 같은 provider 가 구현과 검수를 겸하면 "검수됨"이라고 말할 수 없다(같은 맹점을 공유한다).
  const reviewerIndependent = !reviewer || !reviewer.resolved || !coder || !coder.resolved
    ? null
    : reviewer.provider !== coder.provider;
  ...
  const authOf = (spec.humanApproval && typeof deps._providerAuth === 'function') ? deps._providerAuth : () => 'unknown';
  assignments.forEach(a => { a.auth = a.resolved ? authOf(a.provider) : null; });
  const notAuthed = assignments.filter(a => a.resolved && a.auth === 'no').map(a => `${a.role}:${a.provider}`);

  const blockers = [];
  if (unresolved.length) blockers.push({ code: 'role_unresolved', message: `역할 미설정: ${unresolved.join(', ')} — leerness roles set <역할> --provider <id>` });
  ...
  if (spec.humanApproval && reviewerIndependent === false) blockers.push({ code: 'reviewer_not_independent', message: `검수자와 구현자가 같은 provider(${coder.provider})입니다 — 고위험 작업에서는 독립 검수라고 볼 수 없습니다` });
  return { tier: cls.tier, ko: spec.ko, humanApproval: spec.humanApproval, assignments, unresolved, reviewerIndependent, blockers };
}

function _logPath(root) { return path.join(absRoot(root), '.harness', 'routing-log.json'); }
const _LOG_CAP = 200;
...
function routeCmd(root, taskText, deps = {}) {
  ...
  const cls = classify(taskText, { tier: (arg ? arg('--tier', '') : '') || '' });
  if (!cls.ok) { failJson(json, cls.code, cls.error); return; }
  const p = plan(root, cls, deps);

  const wantDispatch = !!(has && (has('--confirm') || has('--dispatch')));
  const approvedBy = _norm(arg ? arg('--approved-by', '') : '');
  ...
  const rec = {
    at: now(),
    task: _norm(taskText).slice(0, 300),
    tier: cls.tier, confidence: cls.confidence, conflict: !!cls.conflict,
    signals: cls.signals.map(s => `${s.kind}:${s.id}`),
    assignments: p.assignments.map(a => ({ role: a.role, provider: a.provider, model: a.model })),
  ...
  p.assignments.forEach(a => {
    const authMark = a.auth === 'ok' ? ' [인증 확인됨]' : a.auth === 'no' ? ' [🔴 미인증]' : a.resolved ? ' [인증 확인안됨]' : '';
    log(`    ${a.role.padEnd(10)} ${a.resolved ? `${a.provider}${a.model ? ' / ' + a.model : ''}${authMark}` : '⚠ 미설정 — leerness roles set ' + a.role + ' --provider <id>'}`);
  });
  if (p.reviewerIndependent === false) log(`    ⚠ 검수자와 구현자가 같은 provider — 독립 검수가 아닙니다`);
  log(`\n  ⓘ ${_AVAIL_NOTE}`);
...
module.exports = { routeCmd, classify, plan, TIERS, TIER_ROLES, HIGH_RISK, TINY, _logPath, _loadLog };
