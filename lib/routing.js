...
function _norm(s) {
  ...
  let t = raw;
  try { t = raw.normalize('NFKC'); } catch { t = raw; }
  // codex 32차 #3: 제로폭 문자를 끼워 넣으면 영문 키워드가 쪼개져 고위험 규칙을 통째로 빠져나간다
  //   (예: pa<ZWSP>yment re<ZWSP>fund css → tiny). 매칭 전에 서식문자를 제거한다.
...
function routeCmd(root, taskText, deps = {}) {
  ...
  //   승인자는 정규화(NFKC)하지 않은 원문을 쓰되, 보이지 않는 문자만 제거한다 — 제로폭 문자로 채운 값이
  //   "승인자 있음"으로 통과하면 감사 기록에 보이지 않는 승인자가 남는다.
  const approvedBy = String((arg ? arg('--approved-by', '') : '') || '').replace(new RegExp('[\\u200b-\\u200f\\u2060\\ufeff]', 'g'), '').trim();
