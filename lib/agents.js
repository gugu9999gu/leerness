...
const { spawnPortable, spawnPortableSync } = require('./portable-process');
    ...
    for (const agent of EXTERNAL_AGENTS) {
      ...
      try {
        if (agent.id === 'claude') {
          ...
          const r = spawnPortableSync(agent.bin, ['--help'], { encoding: 'utf8', timeout: 4000 });
          ...
          const r = spawnPortableSync('gh', ['auth', 'status'], { encoding: 'utf8', timeout: 4000 });
