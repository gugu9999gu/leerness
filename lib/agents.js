...
function agentsCmd(root, sub, args = [], deps = {}) {
  ...
  if (sub === 'bench') {
    ...
    const ready = EXTERNAL_AGENTS.filter(a => process.env[a.envFlag] === '1')
    ...
    const promises = ready.map(({ agent }) => new Promise((resolve) => {
      ...
      if (spec.unsupported) {
        results.push({ id: agent.id, exit: null, elapsed: 0, stdout: '', stderrLen: 0, error: spec.unsupported, ok: false });
        resolve();
        return;
      }
      ...
      r.on('error', (err) => {
        settle({ id: agent.id, exit: -1, elapsed: Date.now() - t0, stdout: '', stderrLen: 0, error: err.message, ok: false });
      });
    }));
    return Promise.all(promises).then(() => {
      if (has('--json')) { log(JSON.stringify({ task, results }, null, 2)); return results; }
  ...
  if (sub === 'quota') {
    ...
    const results = providers.map((agent) => {
      ...
      } else if (base.auth === 'no') {
        routingEligibility = 'blocked';
        routingReason = 'unauthenticated';
        hintCode = 'restore_authentication';
      } else if (base.auth === 'ok') {
      ...
      return {
        ...
        // Legacy `quota` field and the explicit capacity fields remain null until a verified official adapter exists.
        quota: null,
        quotaState: 'not-observed',
        remaining: null,
        unit: null,
        resetAt: null,
        quotaSource: null,
    ...
    if (has('--json')) {
      log(JSON.stringify({ schemaVersion: 2, observation: 'provider-capacity', quota: results, policy, limitations: limitationCodes }, null, 2));
      return;
    }
...
module.exports = { agentsCmd, _benchLaunchSpec };
