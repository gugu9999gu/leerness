...
const CAPABILITY_SURFACE = {
  filesystem:        { risk: 'low',    desc: '.leerness/ 메타파일 생성·갱신, 변경 전 .leerness/archive/ 자동 백업. 소스코드는 직접 수정 안 함.', descEn: 'Create/update .leerness/ metadata files, auto-backup to .leerness/archive/ before changes. Does not modify source code directly.', optOut: '핵심 동작 (백업으로 보호)', optOutEn: 'core behavior (protected by backups)' },
...
const POWERFUL_COMMANDS = [
  { cmd: 'init',                   note: '.leerness/ 50+ 파일 + .claude hook 생성 (변경 전 백업)', noteEn: 'creates .leerness/ 50+ files + .claude hook (backup before changes)' },
...
const ADAPTERS = {
  claude:   { label: 'Anthropic Claude Code', keys: ['CLAUDE.md', '.claude/commands/handoff.md', '.claude/commands/session-close.md', '.claude/commands/audit.md', '.claude/commands/lazy-detect.md', '.claude/commands/update.md', '.claude/skills/leerness/SKILL.md'], mcp: true, mcpFiles: ['.mcp.json'], mcpEvidence: 'documented' },
  cursor:   { label: 'Cursor', keys: ['.cursor/rules/leerness.mdc'], hookFiles: ['.cursor/hooks.json', '.cursor/hooks/leerness-session.cjs'], mcp: true, mcpFiles: ['.cursor/mcp.json', '.mcp.json'], mcpEvidence: 'documented' },
  copilot:  { label: 'GitHub Copilot', keys: ['.github/copilot-instructions.md'], mcp: false },
  //   1.36.128 (실측): `.mcp.json` 을 둔 디렉터리에서 `codex mcp list` 출력이 대조군과 **완전히 동일**했고
  //   `codex mcp get` 은 "No MCP server named ... found" 를 냈다 — codex 는 프로젝트 `.mcp.json` 을 읽지 않는다.
  //   ⚠ (검수 P2) 처음엔 "~/.codex/config.toml **만** 쓴다" 고 적었는데 그건 측정보다 강한 말이다 —
  //     codex 는 프로젝트 범위 `.codex/config.toml` 도 문서화하고 있다(신뢰 게이트). 내 프로브(임시 디렉터리)에서는
  //     `mcp list/get` 에 안 잡혔지만, 그 명령이 프로젝트 범위를 비추지 않을 수도 있다. **잰 것만** 말한다.
  //   파일을 써 놓고 '등록됨' 처럼 보이게 하는 대신, **실제로 되는 명령**을 안내한다.
  codex:    { label: 'OpenAI Codex CLI', keys: ['AGENTS.md'], mcp: false,
    mcpEvidence: 'measured', mcpWhy: 'codex 는 프로젝트 .mcp.json 을 읽지 않는다 (설정은 codex 자신의 config.toml)',
    mcpSetup: 'codex mcp add leerness -- npx -y leerness mcp serve   (전역 등록 · 프로젝트 범위는 .codex/config.toml 의 [mcp_servers.leerness])' },
  //   1.36.128 (실측): `goose info -v` 가 찍는 실효 설정(extensions)에 프로젝트 `.mcp.json` 의 항목이 없다.
  //   설정 경로도 전역 config.yaml 뿐이다. (provider 미설정이라 세션 기동까지는 확인하지 못함 — 근거 수준을 함께 적는다)
  goose:    { label: 'Goose (Block)', keys: ['AGENTS.md'], mcp: false,
    mcpEvidence: 'measured-partial', mcpWhy: 'goose 는 전역 config.yaml 로 extension 을 관리한다 — 프로젝트 .mcp.json 미인식',
    mcpSetup: 'goose session --with-extension "npx -y leerness mcp serve"  (영구 등록: goose configure)' },
  gemini:   { label: 'Gemini CLI / Antigravity', keys: ['AGENTS.md'], mcp: false },
  //   1.36.128 (실측): `.mcp.json` 을 두면 `opencode mcp list` 에 `✓ connected` 로 나타난다(서버 7 vs 6, 대조군 0건).
  opencode: { label: 'opencode', keys: ['AGENTS.md'], mcp: true, mcpFiles: ['.mcp.json'], mcpEvidence: 'measured' },
...
const OPTIMISM_PATTERNS = [
  ...
  { kind: 'DB',      evidenceRe: /(DB에?\s*저장|insert\s+\d+|데이터베이스|SQL\s*(INSERT|UPDATE|DELETE)|migration|마이그레이션 적용)/i,
    // 1.36.9 (dogfood FP): raw SQL(DDL/DML) + node:sqlite idiom 추가 — SQL-migration 파일만으로 구성된 정직한 done 주장이
    //   '[DB] 호출 흔적 없음' 으로 오탐되던 FP 수정(스캐너의 .sql/.prisma 확장과 짝). 검출 관대화 방향(1.12.4 선례).
    codeRe: /\b(db\.|pg\.|pool\.|mysql\.|mongoose\.|prisma\.|sequelize|knex|sqlite3|MongoClient|createConnection|psycopg|sqlalchemy|cursor\.execute|django\.db|sqlx|gorm|database\/sql|ActiveRecord|jdbc|EntityManager|DbContext|mysqli|CREATE\s+TABLE|ALTER\s+TABLE|INSERT\s+INTO|node:sqlite|DatabaseSync|better-sqlite3)/i,
    label: 'DB 호출' },
...
const BUILTIN_CATALOG = {
  'feature-implementation':       { displayNameKo: '기능 구현 표준 스킬', displayNameEn: 'Feature implementation standard skill', description: '새 기능 구현을 시작할 때 사용 — feature-contract 작성, 재사용 우선 검사, 테스트 증거 수집이 필요한 구현 작업일 때. Use when starting to implement a new feature.', version: '1.0.0', lastUpdated: '2026-05-08', verification: 'unverified', capabilities: ['feature-contracts 작성', '재사용 우선 검사', '테스트 증거 수집', '핸드오프 트리거'], capabilitiesEn: ['feature contract authoring', 'reuse-first checks', 'test evidence collection', 'handoff triggers'] },
  // 1.9.11: 기본 내장 — 로드맵 자동 생성 스킬
  'project-roadmap-generator':    { displayNameKo: '프로젝트 로드맵 자동 생성 스킬', displayNameEn: 'Project roadmap generator skill', description: '프로젝트 진행 상황을 로드맵 한 장으로 시각화해 달라고 할 때 사용 — .leerness 상태를 SVG 트리 HTML 로 뽑을 때. Use when the user asks for a visual roadmap.', version: '0.2.0', lastUpdated: '2026-05-12', verification: 'unverified', capabilities: ['leerness .leerness/* 통합 파싱 (plan/progress/skills/rules/decisions/handoff/current-state)', '좌→우 수평 트리 + 상하 중앙정렬 SVG', '7개 상태 색상 (완료/진행/보류/검토/예정/미완료/오류)', 'design-system + CSS variables 자동 주입', '화이트보드 panning/zoom + 더블클릭 reset', '단일 HTML 출력 (외부 의존성 0)'], capabilitiesEn: ['Parse integrated .leerness/* state (plan/progress/skills/rules/decisions/handoff/current-state)', 'Left-to-right SVG tree with vertical centering', 'Seven status colors (done/in progress/on hold/review/planned/incomplete/error)', 'Automatic design-system and CSS variable injection', 'Whiteboard pan/zoom and double-click reset', 'Single HTML output (zero external dependencies)'] }
};
...
const SECRET_PATTERNS = [
  ...
  // 1.35.14 (시크릿 헌트): Slack app-level 토큰(xapp-, Socket Mode) 보강 — 기존 xox[baprs]- 만으론 미탐(FN). xapp- 는 Slack 전용 prefix라 FP 0.
  { name: 'Slack token', re: /\b(?:xox[baprs]|xapp)-[A-Za-z0-9-]{10,}\b/g },
  // 1.35.14 (codex 헌트 FN #6): ENCRYPTED PRIVATE KEY / PGP PRIVATE KEY BLOCK 등 변종 미탐 → 일반화. PUBLIC KEY/CERTIFICATE 는 'PRIVATE KEY' 없어 미매치(FP 0).
  { name: 'Generic private key', re: /-----BEGIN (?:[A-Z0-9]+ )*PRIVATE KEY(?: BLOCK)?-----/g },
  // 1.35.14 (시크릿 헌트 FN #2): 선두 \b → (?<![A-Za-z0-9]) 로 교체해 복합키(DB_PASSWORD/JWT_SECRET, 키워드 앞 '_')를 포함 — \b 는 '_password' 처럼 word-word 경계라 미탐이었음.
  //   + 키워드 뒤 ["']? 추가 → JSON 따옴표-키("password": "x") 흡수 — 닫는 따옴표가 keyword 와 : 사이에 끼어 기존엔 전부 미탐(config.json 극히 흔함). lookbehind 는 '_' 만 추가 허용 → 신규 FP 0.
  //   + requireSecretLike 신규 적용: JSON 키를 이제 맞추므로 사전 단어 값("secret":"required"/"password":"hashed"/"api_key":"provided")이 노출 → unquoted 와 동일 엔트로피 게이트로 억제(literal 워드리스트 아님, 일관). 실키/디짓포함/len24+ 만 통과.
  //   + 1.35.14 (codex 헌트 FN #3): quoted 키워드에 secret_key/access_key/access_token/auth_token 추가(unquoted 목록과 일관) → Django SECRET_KEY="…" 등 미탐 해소. secret_key 를 secret 앞에 배치(longer-first).
  { name: 'Hardcoded password assignment', re: /(?<![A-Za-z0-9])(?:password|passwd|pwd|secret_key|secret|api_key|apikey|access_key|access_token|auth_token)["']?\s*[:=]\s*["']([^"'\s]{6,})["']/gi, valueGroup: 1, requireSecretLike: true },  // 1.9.365 CV-6: valueGroup → placeholder 오탐 억제
  ...
  // 1.35.14 (시크릿 헌트 FN #1): 선두 \b → (?<![A-Za-z0-9]) — DB_PASSWORD=/REDIS_PASSWORD=/JWT_SECRET= 같은 복합키(.env 관례) 커버. 기존 \b 는 '_' 앞에서 안 걸려 미탐.
  { name: 'Hardcoded credential (unquoted)', re: /(?<![A-Za-z0-9])(?:password|passwd|pwd|secret|secret_key|api_key|apikey|access_key|access_token|auth_token)\s*=\s*([A-Za-z0-9_\-/+]{8,})(?=[\s;,)]|$)/gi, valueGroup: 1, requireSecretLike: true },
...
const MERGE_OVERWRITE_FILES = new Set([
  '.leerness/skill-index.md',
  '.leerness/manifest.json',
  '.leerness/skills-lock.json',
  '.leerness/HARNESS_VERSION',
  '.leerness/LANGUAGE',
  '.leerness/context-routing.md'
]);

// 1.9.380 (UR-0025): 워크스페이스 필수 파일 — verify / migrate audit / migrate apply 단일출처 (harness 3중 중복 제거).
const REQUIRED_WORKSPACE_FILES = ['.leerness/plan.md', '.leerness/progress-tracker.md', '.leerness/guideline.md', '.leerness/protected-files.md', '.leerness/design-system.md', '.leerness/anti-lazy-work-policy.md', '.leerness/session-handoff.md', '.leerness/current-state.md', 'AGENTS.md'];
...
//   한쪽 표기만 아는 사용자/에이전트가 존재하는 기록을 못 찾았다. leerness 도메인어만 담는다(검색 대상이 .leerness 메모리
...
const MINIMAL_SKIP_KEYS = new Set([
  ...
  '.leerness/project-brief.md', '.leerness/task-type-map.md', '.leerness/architecture.md', '.leerness/context-map.md',
  '.leerness/guardrails.md', '.leerness/feature-contracts.md', '.leerness/feature-graph.md',
  '.leerness/testing-strategy.md', '.leerness/review-checklist.md', '.leerness/release-checklist.md',
  '.leerness/session-close-policy.md', '.leerness/language-policy.md', '.leerness/test-evidence-policy.md',
  '.leerness/AX_PLAN_GUIDE.md', '.leerness/AX_MIGRATION_GUIDE.md', '.leerness/AX_NEW_PROJECT_GUIDE.md', '.leerness/AX_SKILL_LIBRARY_GUIDE.md',
  '.leerness/skill-index.md',
  '.leerness/templates/end-of-session-report.md', '.leerness/templates/decision.md', '.leerness/templates/task-row.md',
...
// 1.11.4 (UR-0007): 큐레이션 도구 용어집 카탈로그 — 비개발자용 한 줄 설명. constraints 카탈로그 패턴(aliases+merge-loadable).
