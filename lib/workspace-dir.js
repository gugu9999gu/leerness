...
function migrateLegacyWorkspace(root, opts = {}) {
  ...
  const parsedLockWait = Number(opts.lockWaitMs);
  const lockWaitMs = Number.isFinite(parsedLockWait)
    ? Math.max(0, Math.min(60000, parsedLockWait))
    : 10000;
  ...
  if (!dryRun && fs.existsSync(lockFile)) {
