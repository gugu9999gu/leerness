...
function addReader(operation, root) {
  const id = key(root);
  if (!operation.readers.has(id)) {
    const { createRuntimeCompatibilityReader } = require('./runtime-layout');
    operation.readers.set(id, createRuntimeCompatibilityReader(root));
  }
}
...
function withRuntimeWrites(root, fn, options = {}) {
  ...
  if (parent && !options.fresh) {
    addReader(parent, root);
    check(parent);
    return fn();
  }
  const operation = { readers: new Map(), failure: null };
  addReader(operation, root);
  check(operation);
...
module.exports = { withRuntimeWrites, withRuntimePathWrite, assertCurrentRuntimeWrite,
  installRuntimeWriteInterception, projectWriter, pathWriter, runtimeWriteRoots };
