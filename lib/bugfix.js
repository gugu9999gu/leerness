...
//
// 신뢰 경계(명시): 스토어(.leerness/bugfix-receipts.json)는 **프로젝트가 신뢰하는 설정 파일**이고,
...
function _storePath(root) { return path.join(absRoot(root), '.leerness', 'bugfix-receipts.json'); }
...
function _start(root, id, deps) {
  ...
  if (chk.invalid) { failJson(json, 'store_invalid', `bugfix-receipts.json 형상 무효 — 덮어쓰기 거부\n  · ${_recoverHint(root)}`); return; }
...
// 대상 프로젝트가 cwd 가 아니면 안내 명령에 `--path` 를 붙인다. 1.36.94: 안내가 root 를 잃어서,
//   `--path B` 로 막힌 사용자가 문구를 그대로 복사하면 **cwd 인 애먼 프로젝트 A** 가 변형됐다(실측).
function _pathArg(root) {
  try {
    const abs = absRoot(root);
    if (!abs || abs === process.cwd()) return '';
    return / /.test(abs) ? ` --path "${abs}"` : ` --path ${abs}`;
  } catch { return ''; }
}

// 형상 무효 스토어에서 **동작하는** 회복 절차. 1.36.94: start/receipt/drop/완료 판정이 모두 이 상태를 거부하는데
//   어느 메시지도 빠져나가는 방법을 말하지 않아, 사용자에게 남는 선택지가 보호 해제뿐이었다. 한 곳에서 만들어
//   모든 발화점이 같은 문장을 쓴다(발화점마다 따로 쓰면 다음에 또 한 곳만 고치고 나머지가 남는다).
//   **도구가 대신 지워주지 않는다**: 손으로 고치거나 옮기는 회복은 되돌릴 수 있고 게이트도 살아 있다
//   (실측: 파일을 되돌리면 done 이 다시 보류된다). 도구가 파일을 치우는 편의 명령을 한 번 만들어 봤지만,
//   락 밖 rename·읽기 실패를 손상으로 오인·ID 무시라는 데이터 유실 경로를 함께 들여와서 걷어냈다.
function _recoverHint(root) {
  const f = _storePath(root);
  return `회복: ${f} 를 고치거나(병합 충돌 마커 제거 등) 다른 이름으로 옮기세요 — 옮기면 등록된 probe 가 없는 상태가 되어 완료 게이트는 무영향입니다(파일을 되돌리면 게이트도 되살아납니다)`;
}

// 차단 시 붙는 공통 "탈출구" 안내. 1.36.94: 이 문구가 세 호출부에 복붙돼 있었고, 전부 `bugfix drop <ID>` 를
//   권했다 — 그런데 store_invalid 에서는 그 명령이 같은 사유로 거부되므로 **안내 자체가 막다른 길**이었고,
//   남는 실행 가능한 선택지가 "보호를 끄기" 하나였다. 사유별 판단을 여기 한 곳에 두고 호출부는 렌더만 한다
//   (같은 술어를 표면마다 다시 구현하면 한 곳만 고치고 나머지가 되살아난다 — 1.36.90 에서 겪은 실패).
function escapeHint(code, id, root) {
  //   store_invalid 는 사유 자체가 회복 절차를 담고 있다(checkDoneTransition.lines) — 동작하지 않는 명령을
  //   덧붙이면 사용자를 실패로 유도하고, 보호 해제를 유일한 출구처럼 보이게 만든다.
  if (code === 'store_invalid') return null;
  const p = _pathArg(root);
  return `ⓘ 탈출구: leerness bugfix drop ${id || '<ID>'}${p} (probe 등록 취소) · leerness toggle set bugfix-receipt off${p} (프로젝트 정책)`;
}
...
function checkDoneTransition(root, id) {
  const chk = _loadChecked(root);
  if (chk.invalid) {
    // 회복 경로를 **실제로 동작하는 것**으로 안내한다 — 종전엔 `bugfix drop` 을 권했는데 그 상태에서 반드시 실패했다.
    return {
      blocked: true, code: 'store_invalid',
      message: `bugfix-receipts.json 형상 무효 — 완료 판정을 보류합니다(어느 task 에 probe 가 걸려 있었는지 알 수 없기 때문): ${_storePath(root)}`,
      lines: [_recoverHint(root)],
    };
  }
  const e = chk.list.find(x => x.id === String(id));
...
class BugfixBlocked extends Error {
  //   1.36.94: root 를 함께 싣는다 — 안내 문구가 `--path` 를 잃으면 `--path B` 로 막힌 사용자가
  //   그 문구를 그대로 복사해 **cwd 인 다른 프로젝트**를 건드린다(실측으로 확인된 파괴 경로).
  constructor(res, id, root) {
...
function bugfixCmd(root, sub, rest, deps = {}) {
  ...
  if (sub === 'drop') {
    const chk = _loadChecked(root);
    // 1.36.94 (헌트 #7/#10): 형상 무효 상태에서 **차단 메시지가 안내한 탈출구가 바로 그 사유로 거부**됐다 —
    //   등록·해제·완료가 모두 잠기고, 남는 길은 보호를 통째로 끄는 것뿐이었다(제품이 사용자를 그쪽으로 몰았다).
    //   .leerness 는 git 추적 대상이라 병합 충돌은 현실적인 손상 경로다.
    //   **도구가 파일을 치우는 --force 를 만들었다가 걷어냈다.** 검수에서 그 한 명령이 데이터 유실 경로 5개를
    //   들여온 것이 실측으로 확인됐다: 락 밖 load→rename(동시 writer 가 쓴 정상 스토어를 "손상 원본"이라며 치움 —
    //   자연 경합 실측 19.5%), 읽기 실패(EACCES/EIO)를 손상으로 오인해 유효 데이터 파괴, <ID> 를 무시하는 전역 삭제,
    //   되돌릴 수 없는 게이트 무장해제, 안내가 --path 를 잃어 애먼 프로젝트 파괴.
    //   손으로 고치거나 옮기는 회복은 이 다섯 가지가 전부 없고, 되돌리면 게이트도 되살아난다(실측).
    //   그러니 도구가 할 일은 대신 지워 주는 게 아니라 **어디를 어떻게 고치면 되는지 말해 주는 것**이다.
    if (chk.invalid) {
      failJson(json, 'store_invalid', `bugfix-receipts.json 형상 무효 — 개별 삭제는 할 수 없습니다(어느 항목이 어느 task 것인지 읽을 수 없습니다)\n`
        + `  · ${_recoverHint(root)}`);
      return;
    }
  ...
  if (sub === 'list' || !sub) {
    const chk = _loadChecked(root);
    //   1.36.94: --json 도 회복 절차를 받는다 — 평문에만 넣으면 자동화 소비자는 사유만 받고 빠져나갈 길을 못 받는다.
    if (json) {
      log(JSON.stringify({
        ok: !chk.invalid, invalid: chk.invalid,
        ...(chk.invalid ? { recovery: [_recoverHint(root)] } : {}),
        receipts: chk.list.map(e => ({ id: e.id, repro: e.repro, baselineFailed: !!(e.baseline && e.baseline.failed), rootCause: e.rootCause, siblingScope: e.siblingScope })),
      }, null, 2));
      return;
    }
    if (chk.invalid) { warn(`bugfix-receipts.json 형상 무효: ${_storePath(root)}`); log(`  · ${_recoverHint(root)}`); return; }
...
module.exports = { bugfixCmd, checkDoneTransition, BugfixBlocked, escapeHint, _storePath, _loadChecked, _siblingScopeOk };
