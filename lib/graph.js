...
// ── 📄 문서 탭 (1.36.96, P-0010) — .harness 핵심 문서 7종을 이 파일 안에서 그대로 읽는다.
...
(function(){
  ...
  if(!fl.length){el.innerHTML='<div class="note">문서 없음 — leerness handoff . 로 .harness 를 채우고 재생성하세요.</div>';return;}
...
//   사용자 요청은 "exe 실행기로 .harness 마크다운을 띄우기"였다. 실행기는 런타임 의존성 0 · install script 0
...
const DOC_FILES = [
  { key: 'plan', title: '플랜 · 로드맵', rel: '.harness/plan.md' },
  { key: 'skills', title: '스킬', rel: '.harness/skill-index.md' },
  { key: 'tech', title: '기술 프로필', rel: '.harness/tech-profile.json', lang: 'json' },
  { key: 'handoff', title: '세션 핸드오프', rel: '.harness/session-handoff.md' },
  { key: 'state', title: '현재 상태', rel: '.harness/current-state.md' },
  { key: 'progress', title: '진행 추적', rel: '.harness/progress-tracker.md', tail: true },
  { key: 'decisions', title: '결정 기록', rel: '.harness/decisions.md', tail: true },
];
...
function buildGraphData(root, deps = {}) {
  ...
  // 1.36.96 (P-0010): 📄 문서 탭 — .harness 핵심 문서 7종 스냅샷(마스킹 + 상한 + 잘림 고지).
