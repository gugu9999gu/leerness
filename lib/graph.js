...
<div id="bar">
  ...
  <div id="tabs"><span class="tab on" data-v="graph">그래프</span><span class="tab" data-v="roadmap">로드맵</span><span class="tab" data-v="toggles">⚙ 토글</span></div>
...
<div id="vtoggles" class="view"></div>
<script>
...
// ── 탭 (1.36.30): 그래프 | 로드맵 | ⚙ 토글 ──
var _views={roadmap:document.getElementById('vroadmap'),toggles:document.getElementById('vtoggles')};
...
// ⚙ 토글 뷰 — DATA.toggles (생성 시점 상태). 정적 파일이라 실제 적용은 CLI — 스위치 클릭 시 적용 명령 표시+복사.
(function(){
  var tg=(DATA&&DATA.toggles)||{};var reg=(DATA&&DATA.toggleRegistry)||{};var el=_views.toggles;
  var h='<div class="note">스위치를 누르면 적용 명령이 나타납니다 — 이 파일은 정적 HTML 이라 <b>실제 적용은 아래 명령을 터미널(또는 AI)에게 실행</b>시키세요. AI 는 handoff 헤드라인의 🎛 상태를 자동 준수합니다.</div>';
  Object.keys(reg).forEach(function(id){var on=tg[id]!==false;var m=reg[id]||{};
    h+='<div class="tgrow" id="tg-'+id+'"><div class="sw2'+(on?' on':'')+'" onclick="tgFlip(\''+id+'\')"><i></i></div>'
      +'<div class="tgname">'+esc(id)+'</div><div class="tgdesc">'+esc(m.desc||'')+'<br><span style="opacity:.7">영향: '+esc(m.affects||'')+'</span>'
      +'<div class="cmd" id="cmd-'+id+'"><span id="cmdtext-'+id+'"></span><button onclick="tgCopy(\''+id+'\')">복사</button></div></div></div>';});
  el.innerHTML=h;
  window._tg=tg;
