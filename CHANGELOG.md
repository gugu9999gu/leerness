...
- Git tag와 recentChanges 레코드는 full `refs/tags/vX.Y.Z`와 ref 이름에 금지된 NUL로 구분하고 실제 달력·시간·ISO 시간대 한계(±14:00)를 만족할 때만 반영합니다. lookalike/prerelease/짧은 ref는 최근 변경에도 들어가지 않습니다. HARNESS_VERSION도 canonical/legacy 전체 문자열만 허용하며 `leerness@X+plus@Y`는 round-history/status/handoff 모두 설치 버전 X를 보고합니다. 잘못된 버전 표식과 사람용·기계용 진단값은 CR/LF뿐 아니라 C0/C1·ESC·DEL·Unicode U+2028/U+2029를 한 줄로 안전화합니다. memory는 빠진 하위명령과 잘못된 직접·중첩 하위명령을 구분하고 실제 토큰 및 실행 가능한 선택지를 보고합니다. preview list/show/serve는 손상 JSON·무효 형상·판독 실패를 각각 store_corrupt/store_invalid/store_unreadable로 보고하고 원본을 보존합니다. preview 스키마는 중첩 `history`·`features`·`contract`, 상태 enum, reserved output key, ID 고유성까지 쓰기 전에 검증합니다. `preview add --mockup`도 읽기/승인/서빙과 동일한 realpath 검사를 사용해 바깥 junction/symlink를 저장 전에 거부하고 절대 in-root 입력은 이식 가능한 상대경로로 정규화합니다. `..mockup.html` 같은 정상 파일명과 안쪽을 가리키는 입력 링크는 허용하지만, 생성·임시·정리 출력은 경로 조상 어느 곳의 링크도 허용하지 않아 `.leerness/previews` junction을 통한 프로젝트 밖 쓰기/재귀 삭제를 막습니다. 비정규 preview ID와 저장된 절대·상위·바깥 링크·누락 `mockupPath`는 원본 바이트를 보존한 채 거부하며, 저장 직전 outgoing 형상도 다시 검증합니다. 저장된 title/design/features/history와 오류 토큰의 모든 터미널 제어문자를 중립화합니다. update/self check의 오프라인 unknown 판정과 plan init positional 경로 수정은 기존 회귀로 재확인했습니다.
...
- Windows 전체 E2E의 가짜 Codex를 임의 `.cmd`에서 검증 가능한 npm형 JS shim으로 바꿨습니다. 보안 실행기가 임의 배치 파일을 정상적으로 거부한 뒤 러너에 우연히 설치된 실제 Codex로 폴백하던 Node 22/24 비결정성을 제거했습니다.
...
- 외부 Codex 교차 검수에서 확인한 Windows 자식 env npm_execpath 불일치와 ci init 템플릿 v4 잔존을 수정했습니다. release channel/view/pack/publish 성공·실패, CLI 누락 ENOENT, 셸 메타문자 argv 보존을 실제 명령으로 검증하고 CI에 Node 24/26 전용 런타임 매트릭스를 추가했습니다. 이 실제 npm 경로 프로브는 상위 `LEERNESS_OFFLINE=1` 게이트에서도 대소문자가 다른 상속 `NPM_EXECPATH`를 제거하고 로컬 fake CLI를 단일 키로 명시해 네트워크 없이 같은 계약을 검증합니다.

- GitHub Actions의 checkout/setup-node를 Node 24 기반 v7으로 올리고, Windows 릴리스 경로가 npm.cmd 셸 대신 npm-cli.js를 Node로 직접 실행하도록 바꿔 DEP0190과 셸 메타문자 재해석 위험을 제거했습니다. Linux/macOS는 shell:false npm 실행을 유지하며 전용 release-runtime 프로브를 fast/core/full 게이트에 연결했습니다.
...
- CI 영어 로케일 회귀 수정: selftest의 알 수 없는 플래그 검증을 ko/en 공용 계약으로 통일
...
### `update --check` 가 자기 말을 뒤집었다

```
npm leerness latest: (unavailable — 판정 보류, 다음 실행에서 재조회)
→ up to date
```

바로 윗줄에서 *"모른다"* 고 해 놓고 아랫줄에서 *"최신"* 이라 단정했다. 조회 실패가 판정 분기까지 전달되지 않아, `latest = latest || VERSION` 이 자기 버전을 정답처럼 쓰고 있었다. 이제 결론이 셋으로 갈린다 — `migration_available` / `up_to_date` / **`unknown`**. 모르면 *"확인 불가 — 레지스트리를 조회하지 못했습니다"* 라고 말한다.

같은 자리에서 **`--json` 이 사람용 텍스트를 내던 것**도 고쳤다(`# leerness update` · `Current:` · `Checking npm registry…` 가 그대로 섞여 JSON 파싱이 불가능했다). `--json` 은 `{verdict, registryReachable, latest, current, cli}` 만 낸다.

### `plan init <경로>` 가 그 경로 대신 현재 디렉토리를 채웠다

실측 재현: 대상 디렉토리는 1개 파일 그대로인데 **cwd 가 1개 → 72개**가 됐다. 홈에서 실행하면 홈이 워크스페이스가 된다. `plan` 그룹이 `--path` 만 읽고 positional 은 `list` 만 지원했는데, **파괴적인 `init` 이 그 갭에 있었다**. 이제 준 경로에 적용하고, 없는 경로는 만들지 않고 거부한다.

### 가드

selftest 343 · **변이 6/6 사살**. 가드는 소스 grep 이 아니라 실제 실행으로 검사한다 — 조회 실패 시 결론 문구 · `--json` 파싱 가능 여부 · `plan init` 후 양쪽 디렉토리의 파일 수 · 없는 경로 거부까지 모두 명령을 돌려 확인한다(1.36.101 에서 소스 grep 가드가 세 번 무너진 뒤 세운 원칙).

## 1.36.101 — 2026-08-05 — 명령 표면 전수 감사: 경로 탈출로 남의 파일이 지워지고 있었다

사용자 요청: *"leerness가 적용된 프로젝트에서 leerness 명령어가 의도한대로 동작하는지 테스트 및 디버그도 함께 진행."*

명령 **1,488회**를 신선한 임시 프로젝트에서 실행했다(각 명령의 정상 경로 · `--json` · 인자 없음 · 잘못된 하위명령 · 없는 경로 · `--language en`). 결함 **73건 + 완결성 비평 5건**이 살아남았다(6건은 재현 실패로 반박). 이 릴리스는 그중 **P1 3건**을 닫는다.

### 🚨 skill id 경로 탈출 — 프로젝트 밖 파일이 삭제·덮어쓰기 됐다

```
skill remove "../../../../../victim"  → exit 0 · "✓ user skill removed"
                                      → 프로젝트 밖 디렉토리를 재귀 삭제 (재현 확인)
skill learn "../.."                   → 프로젝트 README.md 4,243바이트 → 125바이트 (재현 확인)
```

`id` 는 사용자 입력인데 검증 없이 `<root>/.harness/skills/<id>` 에 붙었다. 호출부마다 막으면 다음 진입점에서 또 새므로 **id 를 쓰는 모든 함수가 통과하는 단일 병목**에서 거부하고, 파괴적 경로(remove)에는 경로 재확인을 한 겹 더 뒀다. 평범한 id(`open-meteo` 등)는 그대로 동작한다.

### 🚨 "없는 경로에 초록 보고" — 조사한 32개 중 16개

`pulse`·`round-history`·`milestones`·`session-resume` 은 **존재하지 않는 디렉토리**에 exit 0 과 함께 *"✓ 비정상 종료 신호 없음 · R0 · 정상 운영 중"* 을 찍었다. 오타 하나가 안심 보고가 된다. `handoff`·`health`·`status` 는 같은 입력을 이미 exit 1 로 거부하고 있었으니, 절반만 지켜지던 관례다 — 코드 주석에 *"UR-0136 에서 이미 스윕했다"* 고 적혀 있는데도 절반이 남아 있었다(같은 클래스의 3회차).

경로 해석 방식이 명령마다 제각각(`--path` / positional / `_resolveRoot`)이라 개별 수정이 계속 빠졌다 → **디스패처 한 곳**에서 검사한다. 실측: 뚫림 **16 → 0**, 정상 경로 **26/26** 그대로 동작(대조군 — '전부 거부' 하는 구현으로도 뚫림 0 은 나온다).

### 있는데 안 보이던 명령 · 거짓 안내

- 디스패처 126개 중 **46개가 `leerness commands` 에 없었다.** 그중 **`library` 는 `--help` 에도 없었다** — 1.36.98 에서 내가 디스패처에만 붙이고 두 표면에 등재하지 않았다. `gate`·`toggle` 도 같았다. 셋 다 등재하고, 핵심 명령의 등재를 selftest 가 실제 출력으로 확인한다.
- `readme`·`contract`·`self`·`reuse`·`consistency`·`mcp`·`hook`·`llm-bench`·`auto-update`·`ui` 를 하위명령 없이 부르면 **"알 수 없는 명령"** 이라고 답했다. `readme` 는 알 수 없는 명령이 아니라 `readme sync` 가 필요한 명령이다 — 사용자를 오타 찾기로 보내는 거짓 안내였다. 안내 메커니즘은 이미 있었고 **데이터가 4개뿐**이었다.
- 그 두 맵(ko/en)을 모듈 스코프로 올려 **키 집합이 같은지 데이터로 단언**한다. 실행으로만 검사하면 `--language en` 이 프로젝트 밖에서 전환되지 않아 en 맵이 무방비로 남는다(변이에서 en 항목을 지워도 통과했다 — 1.36.89·1.36.97 에 이은 3회차).

### 가드

selftest 343 · 변이 6/8 사살(나머지 2건은 5가지 탈출 시도를 실행해 **등가임을 증명**) · 표면 변이 6/7 · 경로 클래스 정상경로 26/26.

가드를 짜는 동안 **소스 grep 이 세 번 무너졌다**: (1) `const _GROUP_USAGE = {` 문자열이 다른 selftest 의 가드 안에도 있어 슬라이스가 엉뚱한 곳을 집었고 (2) 선언 이름을 바꾸자 옛 가드가 허공을 가리켜 메타가드가 잡았고 (3) 이중가드 확인용 문자열이 **가드 자신에** 있어 자기참조로 통과했다. 그래서 결정을 남긴다 — **표면 가드는 소스 grep 이 아니라 실제 출력·데이터로 검사한다.** 술어만 시험하면 배선이 빠진 것도 못 본다(`skillLearn` 에서 호출을 지워도 통과했다).

### 남은 것 (T-0092~0096 으로 등록)

i18n 누수 13건(`--language en` 무시) · invalid-json 5건 · dead-flag 9건 · false-claim 17건(그중 P1: `update --check` 가 오프라인 조회 실패를 *"최신입니다"* 로 단정 · `plan init <경로>` 가 인자를 무시하고 cwd 에 71파일 워크스페이스 설치) · `glossary build` 2회 실행 시 크래시. 그리고 자체 테스트가 임시 디렉토리를 **79,591개** 누수하고 있다(T-0090).
...
**측정부터 했다.** 렌즈는 7개 도메인이 있었지만 깊이가 극단적으로 치우쳐 있었다 — `database` 12문항 7,193자, `debug` 6문항 538자, 나머지 5종은 각 3문항 125~168자. 11축 중 4축(소유권·생명주기·성능확장성·관측가능성)은 아예 없었고, 사용자가 준 15개 설계 질문 중 실제로 묻는 건 4~5개였다.

결정적인 건 비율이 아니라 **어디서 물렸는가**였다. 직전 라운드(1.36.96)에서 외부 검수 8회가 막은 57건은 계약(378바이트 스텁) · 실패복구(없음) · 신뢰보안(3문항) 축에 몰렸고, **12문항짜리 동시성 축에서 나온 건 0건**이었다. 가장 깊은 렌즈가 실제 작업과 하나도 안 맞았다.

그래서 축 11개를 나열하지 않았다 — 이름만 늘리면 매 세션 읽히는 배경음이 되고, 이 저장소에서 낡은 경고가 무시되는 걸 이미 봤다. `database` 렌즈가 **작동한 이유**만 복제한다: (a) 이름이 아니라 구체적 반례, (b) 파일 내용 기반 자동 소환.
...
**측정 자체가 틀렸던 것도 바로잡는다.** 처음 적은 "recovery 1.4% · 어느 축이든 9.2%" 는 분모에 `.venv`·`site-packages`·생성 번들이 섞여 서드파티 코드로 희석된 값이었다. 손으로 쓴 소스만(벤더·빌드 산출물·미니파이·기계생성 제외) 다시 재면 **19개 프로젝트 3,463 파일에서 recovery 중앙값 6.5% · observability 8.3% · contract 4.0% · database 1.9%, 어느 축이든 31.3%** 다. 7배 낮게 적었던 셈이다. 다만 캡을 고친 지금은 이 비율이 '밀어냄' 이 아니라 '한 줄 더함' 이라 위험도가 다르다.

**신설한 렌즈가 자기 자신에게 먼저 물렸다.** 패키지의 테스트 제외 33개 파일 중 18개에 **주석 없는 `catch {}`** 가 있고 총 911회 나온다(`bin/leerness.js` 427 · `scripts/e2e.js` 378 · `lib/` 28개 중 16개 파일 106회). 상당수는 의도한 방어적 무시겠지만, recovery 5번 문항이 요구하는 "왜 안전한지 한 줄"이 없어 **의도와 사고를 코드만 보고 구분할 수 없다** — 별건(T-0086)으로 스윕한다.

**남의 커스텀 렌즈를 덮지 않는다.** 내장 도메인을 새로 만들면 그 이름을 먼저 쓰고 있던 프로젝트(`.harness/quality-lenses.json`)와 충돌한다. 실측해 보니 질문은 보존되는데 **사용자가 적어 둔 제목이 조용히 사라졌다**('우리팀 계약' → '계약(경계의 약속)') — 내용은 그대로인데 이름만 바뀌니 오히려 더 헷갈린다. 프로젝트가 명시한 title/persona 를 살리되 로케일별로 따로 받는다(ko 제목만 준 프로젝트의 `--language en` 출력에 한글이 새면 안 된다). 덤으로, 절단 경고가 하드코딩하던 "12+커스텀 상한"도 도메인별 실제 내장 문항 수로 파생시켰다(`axes` 는 8 이라 그 문구가 이미 거짓이었다).

**도그푸드가 잡은 것 하나 더.** `verify-claim` 을 이 변경 자체에 걸었더니 recovery 가 뜨지 않았다 — `bin/leerness.js`(1,799KB)와 `scripts/e2e.js`(692KB) 가 512KB 상한에 걸려 **이 저장소의 주력 파일 두 개가 영구히 스캔 밖**이었다. 파일당 4MB 로 올리되 호출당 총 32MB 예산을 함께 뒀다(비용은 파일 하나가 아니라 호출 하나에 묶어야 한다). 그리고 축마다 파일을 다시 열던 것을 **1회 읽기**로 바꿨다 — 같은 파일을 여섯 번 읽으면 IO 도 폭발 위험도 여섯 배다. 검수는 **디렉토리 정션으로 root 밖 파일이 읽히는 것**도 재현했다(문자열 접두만 봤고 최종 파일만 `lstat` 했다). 양쪽을 `realpath` 로 해석해 다시 봉쇄했다 — 1.36.4 부터 있던 구멍이다.

**가드.** selftest 2건 신설(343건). 양성만 재면 항상-참 술어도 통과하므로 **대조군을 함께** 넣었고, 검수가 재현한 오탐(i18n 문구·폴리필 배열·개수 세는 Map·레이 트레이서·`catchPhrase`)과 누락(`@opentelemetry` import)을 그대로 케이스로 박았다. **2차 폭발 회귀 가드**도 넣었다 — 256KB 빈 줄 입력에 시간 예산을 걸어, `\s*` 로 되돌리면 실패한다.

가드가 처음부터 튼튼했던 적은 없다. 변이 1차에서 5개가 살아남았는데 원인이 각각 달랐다: 3개는 **내 변이가 no-op**(문두만 바꿔 판별 문구가 그대로), 1개는 **픽스처가 크기·심볼릭 분기를 지나지 않음**, 1개는 **진짜 구멍 — 영문 표면에 단언이 없어 EN 문항을 한글로 바꿔도 통과**(1.36.89 에서 이미 겪은 구멍이라 `questionsEn` 에 한글 0 을 못박았다). e2e 절제에서도 1개가 살아남았는데 하필 **파생 목록의 존재 이유인 help/commands 세 표면을 아무도 안 보고 있었다**. 심볼릭 링크 단언은 Windows 에서 권한이 없어 조용히 건너뛰어 **주 플랫폼에서 죽어 있었다** — 권한 없이 만들 수 있는 **디렉토리 정션**으로 바꿔 되살렸고, 링크 갈래가 하나도 시험되지 않으면 실패하도록 했다(건너뜀을 통과로 세지 않는다).

## 1.36.96 — 2026-07-31 — 실행기를 만들지 않고 문서를 띄운다 (P-0010)

사용자 요청: *"leerness.html 대신 exe 같은 실행기로 로드맵·그래프·스킬·기술·핸드오프·플랜 마크다운을 그대로 띄울 수 있나? 성능에 영향을 준다면 leerness.html 로 유지하고 거기에 반영."*

**실행기는 만들지 않았다.** exe/Electron 은 런타임 의존성 0 · install script 0 · 사용자 저장소에 실행기 파일 미생성이라는 경계를 깬다 — 그 경계는 1.36.90 에서 테스트로 못박아 뒀다. 사용자가 준 조건("성능에 영향을 준다면 leerness.html 에 반영")이 그대로 답이었다. 같은 목적을 **정적 단일 파일 안에서** 이룬다.

**📄 문서 탭** — `.harness` 핵심 문서 7종(플랜·스킬·기술 프로필·세션 핸드오프·현재 상태·진행 추적·결정 기록)을 `leerness.html` 안에서 그대로 읽는다. 좌측 목록 → 우측 본문, 브라우저만 있으면 된다.
...
- **자격증명 표지가 있는 문서는 본문을 아예 싣지 않는다.** 값 단위 마스킹을 다섯 번 고쳤고 매번 새 누출 경로가 나왔다(아래) — 그래서 **마스킹을 안전의 전제에서 뺐다**. 표지는 값이 아니라 **형식**으로만 정의해 줄 앞 접두어(`> `·`- `·들여쓰기)·대소문자·base64 조성·placeholder 모양에 의존하지 않는다. 마스킹(`redactSecrets`, 대시보드와 같은 술어)은 표지가 없는 문서에 대한 **두 번째 방어층**으로만 남는다.
  - 비용을 먼저 쟀다 — 실제 프로젝트 문서 **156개 중 표지가 걸린 파일 1개(0.6%)**. 사실상 공짜라서 채택했다.
  - 검증: 알려진 경로 **14종**(원본/인용문/목록/들여쓰기/CRLF/END없음 PEM 6 · `~`·`+`·`/` Bearer 3 · 소문자 `bearer` 1 · `AUTH=` 뒤 토큰 1 · placeholder 모양 1 · 반복 문자 1 · DB 접속문자열 1) 전부 본문 미포함. 이후 검수가 보인 **네 형태**(RFC6750 짧은 Bearer · 쿠키 · compact JWE · 강조로 감싼 스킴)도 같은 술어에서 검출된다. 표지 없는 문서는 그대로 실린다.
...
- 실측(이 저장소): 문서 탭이 `leerness.html` 에 더하는 양은 **+39.2KB**(120.8KB → 159.9KB). 자동 생성 기본값은 바꾸지 않았다.

**만들면서 세 번 틀렸고, 셋 다 다른 방식으로 드러났다.**

1. **코드스팬 자리표시자가 본문의 모든 숫자를 삼켰다.** 자리표시자를 감싸는 문자를 빠뜨려 복원 정규식이 `/(\d+)/g` 가 됐고, `1.36.96` 같은 평범한 숫자가 코드로 바뀌었다. 게다가 그 문자를 **원시 제어문자(U+0001)로** 소스에 심어 놓아 눈에 보이지도 않았다 — 명시적 ``\u0001`` 이스케이프로 바꿨다.
2. **전체 상한 512KB 가 구조적으로 도달 불가능했다.** 7종 × 64KB = 448KB 라 `skipped` 분기가 한 번도 실행되지 않았고, 그걸 검사하던 `every` 는 빈 컬렉션 위에서 공허하게 참이었다. 상한값을 주입 가능하게 해 분기를 실제로 발화시켰다(측정: skipped 3건). 그 과정에서 **159자짜리 쓸모없는 조각**이 "잘림"으로 표시되던 것도 발견해, 잔여 예산이 의미 없으면 조각 대신 "담지 않았다"로 바꿨다.
3. **좁은 화면에서 레이아웃이 무너졌다** — 190px 고정 내비가 자리를 다 먹어 본문이 한 글자씩 세로로 흘렀다. 테스트로는 보이지 않는 종류라 **실제 브라우저로 열어 봐야** 잡혔다. 760px 이하에서 내비를 가로 칩으로 접는다(실측: 375px 에서 본문 폭 90 → 323px, 가로 스크롤 0).

**그리고 검수가 출하를 막았다 — 10건 중 7건이 재현됐고 전부 고쳤다.**
...
- **P2 빈 경계 줄 하나가 문서 전체를 지웠다.** 선두 빈 줄 + 초장문 한 줄이면 `keep` 은 1 이 되는데 담긴 내용은 0자였다 — **70,000자 문서가 빈 화면**이 됐다(누적 로그는 끝 개행 하나로 같은 일이 났다). 줄 수가 아니라 담긴 **내용**으로 판정하고, 없으면 경계의 내용 있는 줄을 코드포인트로 자른다.
- **P2 "64KB/512KB" 가 사실이 아니었다.** UTF-16 코드 단위를 세면서 KB 라고 적어, 한글 문서에서 합계 **1,152KB**(HTML 1.21MB)까지 실렸다. 상한을 UTF-8 바이트로 바꿔 주장을 참으로 만들었다.
- **P2 링크 정규식이 O(n²) 였다.** 닫히지 않은 대괄호마다 남은 문자열 전체를 다시 훑어 64KB 입력에서 **1,825ms**(최악 2,730ms) 브라우저 메인 스레드가 멈췄다. 수량자를 유계로 바꿔 **41ms**(최악 23ms).
- **P2 역슬래시 프로토콜 상대 URL 이 통과했다.** 슬래시 2개만 막았는데 브라우저는 역슬래시를 슬래시처럼 읽는다 — `\\host\share` 가 링크가 됐다.
- **P2 e2e 가 계약을 깨는 변이 3종을 통과시켰다.** 허용 링크는 HTTPS 하나만, 제목은 H2 만 검사했고, `tech`·`progress` 두 문서는 아무것도 확인하지 않아 경로가 끊겨도 초록이었다.
- **P3 링크를 만든 뒤 강조 정규식이 href 까지 다시 해석했다** — `href="https://a/<b>b</b>/c"`. 완성된 앵커를 자리표시자로 빼 둔다. 문서에 실제로 U+0001 이 있으면 자리표시자와 충돌하던 것도 함께 막았다.
- **검수가 틀린 것도 있었다.** SendGrid "누출"은 세 번 연속 **내 픽스처**가 틀린 것이었고(뒤가 38자 → 같은 글자 반복 → 39자) 코드는 처음부터 정상이었다. 길이를 손으로 세지 말고 생성하도록 바꿨다. 마스킹 오탐 1건(`password = 사용자가…`)도 1.36.95 규칙만으로 재현해 **회귀가 아님**을 확인했다.

**그리고 재검수가 다시 막았다 — 6건 중 3건이 위 수정이 만든 새 결함이었다.**

- **P1 END 줄이 없는 개인키는 본문이 그대로 남았다.** "블록 전체를 지운다"는 규칙이 `-----END …-----` 를 요구했는데, 붙여넣다 잘린 키가 오히려 흔하다. BEGIN 뒤에 이어지는 base64/헤더 줄까지 함께 지운다.
- **P2 빈 경계 줄이 두 개면 같은 데이터 소실이 재발했다.** 내가 "담긴 내용으로 판정"한다며 쓴 `acc` 에는 **줄바꿈 바이트가 포함**돼 있었다 — 빈 줄 두 개면 `acc=1` 이 되어 판정을 통과하고 70,000자가 줄바꿈 한 글자로 줄었다. 내용 바이트를 따로 센다. 픽스처를 빈 줄 **한 개**로만 만든 탓에 못 봤다.
...
- P3 링크 길이 상한이 **이스케이프 후** 길이에 걸려 원문 796자짜리 정상 쿼리 URL 이 거부됐다(`&`→`&amp;` 팽창). 실측 비교로 라벨 300 / URL 4,000 을 골랐다 — 최악 115ms(무계는 1,747ms).
- P3 잘림 고지의 "자" 가 UTF-16 코드 단위였다 — 이모지 문서에서 `32,767/60,001자` 로 표시되지만 실제는 `16,384/30,001자`. 코드포인트로 센다.
- **내 실측이 "신규 오탐 0" 이라고 말했는데도 오탐이 실재했다.** 실제 문서 189개에 그런 문장이 없었을 뿐이다 — **실제 데이터 측정은 구성된 반례를 대체하지 못한다.**

**세 번째 검수에서 7건이 더 나왔고, 같은 자리에서 세 번 틀린 것이 있었다.**

- **P1 암호화 개인키의 본문이 샜다.** 암호화 키는 `Proc-Type:`/`DEK-Info:` 헤더 뒤 **빈 줄**이 오고 본문이 시작하는데, 규칙이 빈 줄에서 멈췄다. 반대로 아무 `Name: value` 줄이나 먹으면 BEGIN 을 인용한 정책 문서의 산문을 삼킨다 — 헤더는 **PEM 표준 이름만** 허용하고 본문 base64 줄이 최소 하나 있을 때만 확장한다.
- **P1 무관한 산문을 통째로 삼키는 경로가 있었다(변이 실험 중 발견).** `[\s\S]*?` 로 BEGIN 과 END 를 잇는 규칙은 그 사이의 **아무 내용이나** 먹는다 — 종료되지 않은 키 하나가 문서 뒤쪽의 END 줄과 짝지어지면 사이의 모든 문단이 사라진다. 사이에는 PEM 헤더·base64·빈 줄만 허용하도록 좁혔다.
- **P2 영문자만으로 된 32자 Bearer 토큰과, 구분자 없는 AWS 실값이 샜다.** 앞 라운드에서 오탐을 막으려고 건 조건이 실값까지 놓쳤다. Bearer 는 "숫자가 있거나 32자 이상", AWS 문맥은 패턴을 되살리되 **꼬리 값만** 치환한다(문장 구조는 남는다 — 커밋 해시가 함께 가려지는 것은 감수한다. 누출보다 낫다).
- **P2 데이터 소실이 세 번째로 재발했다 — CRLF 의 `\r`, 공백, 탭이 "내용"으로 세어졌다.** `keep > 0` → `acc > 0` → `content > 0` 으로 세 번 고쳤고 매번 셈을 조금씩 틀렸다. **같은 자리에서 세 번 틀렸다면 픽스처가 아니라 접근이 틀린 것이다.** 셈을 없앴다 — 줄 단위로 담은 것과 경계 줄을 코드포인트로 자른 것을 **둘 다 만들어 긴 쪽**을 쓴다. 어떤 공백 조합에도 예산을 채우는 쪽이 이긴다.
- **P2 URL 스킴 판정이 C0 제어문자로 우회됐다.** 탭·개행만 지웠는데 `\x03`·`\x01`·`\x00`·`\x1F`·`\x7F` 를 앞이나 중간에 끼우면 통과했다. C0 전체와 DEL 을 제거한다.
**네 번째 검수에서 또 7건 — 그리고 "A를 고치면 B가 깨지는" 형태가 두 번 나왔다.**

- **P1 20~31자 영문 Bearer 토큰이 샜다.** 앞 라운드에서 오탐을 막으려 건 길이 문턱(32자)이 실제 토큰을 놓쳤다 — OAuth 토큰 길이는 보장되지 않는다. **길이가 아니라 모양**으로 가른다: 낱말을 이어 붙인 camelCase(각 조각 3자 이상)만 산문으로 보고 나머지는 가린다. `authenticationMiddleware` 는 조각이 14/10자라 산문, `AbCdEfGhIj…` 는 조각이 2자라 토큰이다.
- **P2 절단이 문서의 제목·푸터를 버렸다.** "줄 단위 결과와 코드포인트 절단 결과 중 긴 쪽" 이라는 앞 라운드의 수정이, 경계 줄이 길면 **앞부분을 통째로 버렸다** — `TITLE` + 초장문이면 `TITLE` 이 사라지고 화면 고지의 "앞부분만" 이 거짓이 된다. 온전한 줄로 채운 뒤 **남은 예산만큼 경계 줄을 이어 붙인다**. 그런데 그렇게만 하면 이번엔 평범한 문서까지 줄 중간에서 잘렸다 — **혼자서는 어떤 예산으로도 담길 수 없는 줄**일 때만 이어 붙이도록 좁혀 두 성질을 함께 만족시켰다.
- **P2 BEGIN 인용 뒤의 평범한 영문 한 줄을 개인키 본문으로 오인해 지웠다.** 본문 요건이 16자였다. 40자로 올렸더니 이번엔 **줄이 짧게 감긴 키의 본문이 샜다**(내 회귀 확인에서 22자·16자 본문이 남았다). **길이가 아니라 내용**으로 가른다 — 진짜 base64 는 숫자나 `+ / =` 를 포함하고, 영문 낱말을 이어 붙인 식별자는 포함하지 않는다.
- P3 PGP 블록은 END 앞 `=CRC` 줄을 허용하지 않아 CRC 와 END 가 남았다.
- **두 번 다 "더 센 조건" 이 아니라 판별 기준 자체를 바꾸는 것이 답이었다.** 길이 문턱을 올렸다 내렸다 하면 한쪽이 반드시 새거나 파괴된다.

**다섯 번째 검수가 그 "판별 기준"마저 양방향으로 깼다 — 그래서 원칙을 정했다.**

- **P1 실제 RSA 키의 영문자 전용 본문 줄에서 51줄이 그대로 노출됐다.** 검수가 진짜 키를 생성해 16자로 감싸니 영문자만인 줄이 나왔고, "진짜 base64 는 숫자나 `+ / =` 를 포함한다"는 내 판별이 무너졌다. 반대 방향도 함께 무너졌다 — `PolicyVersion2026Final` 은 숫자를 포함한 22자 영숫자라 본문으로 분류돼 문서가 훼손됐다.
- **P1 `Bearer abcdefghijklmnopqrst` · `Bearer CorrectHorseBatteryStaple` 같은 유효한 불투명 토큰이 산문으로 판정돼 노출됐다.** 낱말꼴 예외를 둔 순간 **그 예외가 곧 누출 경로**가 됐다.
- **다섯 번 시도해 매번 한쪽이 깨졌다면, 그건 판별의 문제가 아니라 원칙의 문제다.** 값의 조성으로 시크릿과 산문을 가르려는 시도에는 반드시 반례가 있다. 그래서 원칙을 정했다 — **표시용 마스킹은 안전한 쪽으로 실패한다.** 값이 아니라 **문맥**으로 판정한다: `BEGIN` 마커 뒤의 **줄 전체에 공백이 없는 줄**은 키 재료로 본다. 산문은 거의 항상 공백을 포함하므로 문단은 살아남고, 어떤 조성의 키 본문도 새지 않는다. Bearer 도 예외를 없앴다.
- **대가는 명시한다.** BEGIN 을 인용한 문서에서 **바로 뒤의 단일 토큰 한 줄**이 가려지고, `Use Bearer authenticationMiddleware for API requests.` 는 `Use *** for API requests.` 가 된다. 원본 파일은 그대로이고 이건 표시용 스냅샷이므로, 누출보다 이쪽이 낫다. (20자 미만은 카탈로그가 후보로 잡지 않으므로 `Bearer 인증` 같은 표현은 영향이 없다.)
- 검증: 진짜 RSA 키를 생성해 16자·64자 두 방식으로 감싸고 END 유무까지 조합해 **본문 0줄 누출**을 확인했다.

**여섯 번째 검수가 그 원칙마저 뚫었다 — 그래서 마스킹을 안전의 전제에서 뺐다.**

- **P1 Markdown 인용문(`> `)으로 감싼 실제 개인키가 전량 노출됐다.** 내 규칙은 줄 앞에 공백·탭만 허용했다. README·정책 문서에서 키를 blockquote 로 인용하는 것은 자연스러운 마크다운이다. 16자 래핑에서 51/51, 64자에서 13/13 줄이 그대로 실렸다.
- **P1 Bearer "예외 없음" 원칙에도 누출 경로가 넷 남아 있었다** — 카탈로그가 대문자 `Bearer` 만 받고 `~`·`+`·`/` 를 문자군에서 빠뜨렸으며, `AUTH=Bearer <토큰>` 은 앞부분만 가려 `AUTH=*** <실토큰>` 이라는 **거짓 안심**을 만들었고, placeholder 모양 토큰은 가드가 통과시켰다.
- **여섯 번을 고쳤는데도 새 경로가 나온다면, 고치는 방향이 틀린 것이다.** 값 단위 마스킹이 완벽해야 안전한 구조를 버렸다 — **자격증명 표지가 하나라도 있으면 그 문서의 본문을 싣지 않는다.** 이제 마스킹이 부분적으로 실패해도 누출로 이어지지 않는다.
- **변이가 5건 살아남아 픽스처의 결함도 드러났다** — 시크릿을 한 파일에 몰아넣으면 **표지끼리 서로를 가려 줘서**, 특정 표지를 무력화하는 변이가 그대로 통과했다. 표지 술어는 **하나씩 격리**해 단언한다(7/7 검출). 남은 생존 2건은 `_SECRET_BEARER` 가 먼저 가려 등가가 된 것이라 목록에서 뺐고, 마스킹 제거 변이는 "가드를 통과한 문서에도 두 번째 층이 살아 있는가" 단언을 넣어 잡았다.

**일곱 번째 검수가 가장 아픈 것을 짚었다 — 내가 표면 하나만 보고 안전을 선언했다.**

- **P1 문서 탭을 가드해도 같은 내용이 `nodes`·`roadmap` 으로 그대로 나갔다.** `decision add "Authorization: Bearer <토큰>"` 이 `DATA.nodes[].label` 과 `detail.decision` 에 원문으로 실렸다(실측). 그래프 쪽에는 **마스킹조차 없었다.** "문서 본문을 통째로 제외하므로 마스킹 실패가 누출로 이어지지 않는다"는 내 전제가 성립하지 않았다.
  - 필드마다 손으로 감싸면 반드시 빠뜨린다(그래서 문서 탭만 덮였다). **임베드 직전 한 곳**에서 모든 문자열을 같은 술어로 통과시킨다 — 지금 있는 표면도, 나중에 추가될 표면도 함께 덮인다.
  - 실측: decision·task·plan 세 경로 모두 생성 HTML 과 `DATA` 전 경로에서 0건. 정상 항목은 그래프에 그대로 남는다.
- **P1 표지를 빠져나가는 네 형태** — RFC6750 의 짧은(19자) Bearer, 세션/Set-Cookie, compact JWE(5분절이라 `eyJ…eyJ` 가 아니다), 마크다운 강조로 감싼 `` `Bearer` ``. 넓히기 전에 비용을 쟀다: 실제 문서 156개에서 **추가로 걸리는 파일 0개**라 셋 다 넓혔다.
**여덟 번째 검수에서 P1 이 처음으로 0 이 됐고, 남은 P2 는 내가 방금 넣은 코드가 원인이었다.**

- **깊이 처리를 세 번 바꿨고, 두 번째 진단이 틀렸다.** ① 상한 12 + **원본 통과** → 12단계 밖이 정규화를 우회(누출). ② "상한이 우회를 만든다"고 보고 **상한을 제거** → 약 24KB 짜리 유효 JSON 하나로 **생성 자체가 실패**(직렬화가 4천 단계에서 스택을 터뜨린다). ③ 상한 200 + **자리표시자 대체** → 8천 단계까지 생성 성공, 누출 0. **문제는 상한이 아니라 상한을 넘겼을 때 원본을 통과시킨 것이었다 — fail-open 이면 우회, fail-safe 면 보호다.**
- **순환을 막으려 쓴 방문 집합이 정상 데이터를 지웠다.** 전역 집합으로 두면 같은 객체를 두 필드가 공유할 때 두 번째가 순환이 아닌데도 `null` 이 된다(실측). 재귀 스택처럼 **들어갈 때 넣고 나올 때 빼야** 진짜 순환만 끊긴다.
- **CRLF 문서를 앞에서 자를 때 고립된 `\r` 이 남았다** — `\n` 으로만 나누므로 줄 끝의 `\r` 이 내용으로 세어지고, 재조립 때 개행이 붙지 않으면 제어문자가 표시 데이터에 남는다.
- **순서 목록 번호가 조용히 바뀌었다** — `2. two` 가 브라우저에서 `1. two` 로, `1·3` 이 `1·2` 로 보였다. 문서의 의미를 바꾸는 것이라 원문 번호를 보존한다. **표 정렬**도 문법으로는 받으면서 의미를 버리고 있었다 — 받은 이상 반영한다.
- **변이 진단에서 픽스처가 판별점을 비껴간 경우가 셋 더** — CRLF 는 경계 줄이 이어붙어 정상 CRLF 가 되고(상한 안에 들어가는 줄로 교체), 순환은 깊이 상한이 먼저 끊어 주며(출력 크기까지 단언), 깊이 상한은 "생성 성공"만 보면 낮춰도 통과한다(보통 깊이 데이터 보존까지 단언).

- **변이 진단에서 픽스처 함정이 둘 더 나왔다** — 누출 needle 을 `Authorization: Bearer …` 로 만들었더니 **마스킹만으로도 지워져** 표지 가드 제거 변이가 살아남았고(쿠키 형태로 교체), Bearer 문자군 판별 픽스처는 `/`·`~` 를 맨 앞에 둬서 **접두 허용이 흡수**해 버렸다(가운데 배치 + 양쪽 16자 미만으로 교체). 둘 다 가드가 약한 게 아니라 픽스처가 그 경로를 판별하지 못한 경우였다.

- **변이 생존 5건의 원인이 세 갈래로 갈렸다** — 등가 변이 1건(가림 제외 목록이 비어 분기가 무의미), **픽스처 간섭** 3건(한 문서에 PEM 을 몰아넣어 서로를 가려 줬다 → 마스킹 단언을 격리 입력으로 직접 검증), **픽스처가 분기에 도달하지 못한** 1건(산문 뒤 base64 가 없어 규칙이 발동조차 안 했다). 앵커가 유일하지 않아 엉뚱한 규칙을 고치던 것도 유일성 검사가 잡았다.

- **마스킹을 넓혔으니 실제 데이터로 오탐을 쟀다.** 로컬의 실제 leerness 프로젝트 문서 **189개 · 10,431줄**에 걸어 본 결과, 마스킹된 72줄은 **전부 1.36.95 규칙으로도 이미 마스킹되던 줄**이었다 — 이번 변경이 새로 건드린 줄은 **0건**, 반대로 커버리지가 후퇴한 줄도 **0건**. (그 72줄에는 `idempotency_key=run:phase:step` 같은 **기존 규칙의 오탐**이 섞여 있다. 이번 회귀가 아니므로 별도 항목으로 남겼다.)
- 검증: 게이트 exit 0 · e2e **416/416** · 신규 1블록 · 변이 **81종 81/81**(등가 2건 제외 · 앵커 문제 0) · 자체 헌트 24종 결함 0 · 출구 전수 13종 · 계약 25종(원시 HTML · 엔티티/제로폭 URL · 표 구분줄 오인 · 미종료 펜스 · 주석 · CDATA) · 성능 6종. 실제 브라우저(788px·375px)에서 탭 전환·7종 내비·본문 렌더 확인.
- **변이 1차에서 4건이 살아남았고 원인이 넷 다 달랐다** — 변이 하네스의 앵커가 유일하지 않아 엉뚱한 함수를 고친 것(하네스 버그), `mdInline` 도 내부에서 이스케이프하므로 XSS 단언만으론 못 잡는 것(단언이 목적이 아니라 부작용만 봄), 이모지만 반복한 픽스처가 서로게이트 쌍 경계에 정확히 맞아 쪼갤 자리가 없던 것, 줄 경계 단언이 tail 문서에만 걸려 non-tail 변이를 못 본 것. 앵커 유일성 검사를 러너에 넣고 나머지 셋은 픽스처를 고쳤다.
- **계약 검증 도구 자신이 오탐 5건을 냈다.** 출력 전체에서 `on...=` 를 찾아 `&lt;img src=x onerror=1&gt;` 처럼 **이미 이스케이프된 글자**를 속성으로 오인했다. 그대로 믿었으면 없는 버그를 고쳤을 것이다. 실제 태그 안만 보도록 고치고, **탐지기가 진짜 위반을 잡는지 대조군으로 먼저 증명**한 뒤 "위반 0"을 말한다.
- **게이트 실패 한 번은 내가 만들었다.** 게이트 실행 중에 소스를 고쳤고, 그 편집이 `String.raw` 템플릿 안 주석에 백틱을 넣어 템플릿을 끊었다 — 바로 그 위에 "여기선 백틱을 쓸 수 없다"고 적어 둔 자리였다. 처음엔 동시 실행 간섭이라고 생각했지만 격리 재현으로 5초 만에 진짜 원인이 나왔다.
- **검수가 살려낸 마지막 변이는 "데이터는 맞는데 화면이 틀리는" 경우였다.** 코드포인트 단언을 데이터에만 걸어 둬서, 화면 고지를 `String.length` 로 되돌리는 변이가 통과했다. 사용자가 보는 것은 화면이다 — 생성물에서 고지가 어느 값을 쓰는지까지 확인한다.

## 1.36.95 — 2026-07-31 — AI 에게 매 세션 읽히던 파일의 절반이 틀린 과거 수치였다

사용자 질문에서 시작했다 — "Opus 5 처럼 모델에 검증이 내장돼 있으면 하네스가 검증을 또 시키는 건 낭비 아닌가. 구동 모델을 인지해 맞춤으로 알려줄 수 있나?" 조사 결과 **모델 인지는 만들지 않기로 했고**(아래), 대신 그 조사가 훨씬 큰 것을 드러냈다.

**출하 중인 한글 `session-workflow.md` 템플릿 10,308자 중 5,216자(50.6%)가 낡은 릴리스 변경이력이었다.** 헤딩도 없이 Memory CRUD 섹션 아래 붙어 있었고, 내용은 MCP 도구 수를 18 → 42 로 세어 올라가는 로그였다 — **실제 도구 수는 89 다**. 이 파일은 AGENTS.md 의 필독 1번이고 CLAUDE.md 가 "⭐ 매 세션 첫 행동"으로 가리키는 파일이다. AI 가 매 세션 읽도록 설계된 문서의 절반이 틀린 과거 수치였다. 영문 템플릿은 **이미 깨끗했다**(`## Auto-recovery & security` 로 정리돼 있었다) — 한쪽 표면만 썩어 있었고 아무도 비교하지 않았다.

- 이력 43줄을 제거하고, 남긴 운영 사실 11줄에는 영문과 대응하는 `## 자동 회복 · 보안` 헤딩을 줬다. 실측: 한글 템플릿 **10,308자 → 5,107자**, 영문 5,075자 — 두 언어 크기가 맞았다.
- e2e 가 **도구 수 주장 자체가 0 인지**를 양쪽 언어에서 단언한다. 개수를 고쳐 적는 것은 다음 릴리스에 또 낡는다. (초안의 정규식 `\d+\s*도구\b` 는 **한글 뒤 `\b` 가 절대 매치되지 않아** 아무것도 세지 않는 공허 단언이었다 — 변이 테스트가 잡았고, 그 함정은 이미 내 메모리에 적혀 있었는데도 다시 밟았다.)

**기존 설치본의 이력까지 지우는 기능은 이 릴리스에서 뺐다.** 템플릿에서 줄을 지우면 머지(`_managedMerge`)가 그것을 "사용자 커스텀"으로 보고 `## Preserved previous content` 로 이월하므로, 원래는 기존 설치본의 이력도 함께 처리하려 했다. 세 가지 설계를 시도했고 전부 사용자 데이터를 지웠다:

1. **휴리스틱**(버전 접두 + 릴리스노트 표지) — 마크다운을 쓰는 사용자 문장을 삭제했다(한국어 코퍼스 34/34, 다국어 5/5). 백틱 하나가 보존을 삭제로 뒤집었다. 내가 댄 근거 "실제 설치본 21개에서 오탐 0건"은 **그 21개가 전부 작성자 본인의 도그푸딩 프로젝트라 제3자 산문이 표본에 없어서** 무효였다.
2. **기계 생성 정확 일치 목록** — 오탐 면적은 크게 줄었지만 `trim` 비교가 들여쓴 인용을 지웠고, 들여쓰기를 배제하자 이번엔 우리가 2칸 들여쓴 채 내보낸 줄을 못 지웠으며(설치본 21/21 · 42회), `raw === l` 비교는 **CRLF 파일에서 `\r` 때문에 121/121 을 전부 누출**시켰다(Windows 가 주 대상인데 기능 전체가 죽는다).
3. **원문 그대로 비교** — 여기까지 와서도 `<details>` 가 파일 어디에든 있으면 무관한 줄의 리터럴 펜스가 변형됐고, 템플릿이 들여쓴 줄을 사용자가 안 들여쓰면 삭제됐다.

**근본 원인은 하나다: 문자열로는 "우리가 내보낸 줄"과 "사용자가 그 줄을 적은 것"을 구분할 수 없다 — 바이트가 같다.** 규칙을 정교하게 만들수록 한쪽을 고치면서 다른 쪽이 깨졌다. 진짜 해결은 생성 영역 마커(provenance)이고, 그건 `_managedMerge` 재설계라 별도 라운드로 다룬다.

**그리고 "삭제하지 않는다"를 실제로 참으로 만들기 위해 기존 필터 하나를 걷어냈다.** 범위를 좁힌 뒤에도 검수가 반증했다 — 템플릿에서 이력 43줄을 지우면 그 줄들이 **처음으로 머지 경로를 타게 되고**, 1.36.60 이 넣어 둔 자동생성 이력 필터(`- .*MCP **N 도구` 등)가 그때 발화한다. 실측: 설치본 21개의 이력 731줄 중 **346줄 삭제**, 그리고 사용자 문장 `- 사용자 메모: MCP **77 도구** 는 우리 내부 표현이며 삭제 금지` 도 함께 사라졌다. 원래 주석은 "사용자 산문이 이 패턴과 겹칠 확률은 무시 가능"이라고 적고 있었다. 그 필터를 제거했다 — 이력 줄은 삭제되지 않고 `Preserved` 로 이월된다.

- 실측(실제 설치본 21개, 진짜 `_managedMerge`): **유실 고유 줄 0**. 검수가 만든 사용자 문장 반례 5종 전부 보존. 이력 줄은 이월(삭제 아님). 멱등. **검수가 독립 재현으로 확인했다** — 21/21 유실 0, 같은 입력에 1.36.94 코어를 쓰면 정확히 346줄 유실.
- 대가는 **과보존**이다 — 기존 설치본의 `session-workflow.md` 는 줄지 않고, 이력이 `Preserved` 블록으로 자리를 옮긴다. 실측: 설치본 21개 합계 203,408 → 245,239자(**+20.6%**), 파일별 1.03~2.17배. 멱등이라 반복 성장은 없고 언어 전환(ko↔en)·다른 관리 파일에도 부작용이 없다. 이 파일의 계약(false-DROP 이 버그, false-PRESERVE 는 무해)에 맞는 방향이고, 실제 정리는 provenance 라운드에서 한다.
- 즉 이 릴리스의 효과는 **신규 `init` 은 즉시 깨끗해지고, 기존 설치본은 데이터를 잃지 않는다** 까지다.

**`handoff` 의 라벨과 동작이 어긋나 있었다.** `Decisions (last 40 lines)` 라고 써 놓고 파일 전체를 넣은 뒤 **앞** 4,000자를 잘랐다 — 단위(줄↔문자)도, 방향(마지막↔처음)도 반대였다. 라벨대로 뒤에서 N줄을 취한다. 이 한 줄짜리 수정이 **세 번의 회귀**를 거쳤고 전부 검수가 실행으로 잡았다:

- 1차: 섹션 전체를 `slice(-4000)` 했더니 `=== Decisions … ===` **헤더가 1순위로 잘려** 결정 내용이 앞 섹션 꼬리에 붙었다(실사용 50개 중 4개 발화). 상한은 **본문에만** 적용하고 헤더는 불가침으로 바꿨다.
- 2차: 줄 단위 절단을 매 반복 `join()` 으로 구현해 **O(n²)** 가 됐다(48,000줄에서 30초대, 입력 2배마다 4배). 누적 길이를 한 번에 계산하도록 고쳤다.
- 3차: 한 줄이 상한보다 길 때 문자 인덱스로 잘라 서로게이트 쌍이 쪼개졌고(평문 U+FFFD · JSON 단독 surrogate `0xDE00`), 빈 파일을 `1/1 줄` 이라 표시했고, 정확히 4,000자인 본문을 초과로 오판해 절반을 버렸다(마지막 줄에도 개행을 셌다).

- 잘릴 때도 라벨이 사실이어야 한다 — `25/121 줄 (상한으로 잘림)` 처럼 실제 표시 줄 수를 함께 적고 **줄 경계**에서 자른다.
- **`handoff --json` 은 같은 내용을 앞 8,000자로 잘랐다** — 같은 명령의 두 표면이 반대 방향이었고, 외부 AI/MCP 용이라고 문서화된 쪽이 가장 오래된 결정만 받았다(15MB `decisions.md` 에서 최신 미포함). 누적 로그(progress-tracker/decisions/task-log)는 뒤를 남기고, 그 외는 앞을 남긴다. `truncated`·`totalChars`·`kept` 를 함께 준다. 마커까지 포함해 8,000자를 넘지 않는다.
- 실측: 결정 120개 중 081~120 이 **끊김 없이 연속**, Task Log 200개 중 최신 60개(141~200)만, `--json` 에 최신 포함·최초 잘림. 성능은 **O(n²) 폭증이 사라지고 선형 범위**다 — 파일을 읽어 `split` 하므로 입력에 비례하는 비용은 남는다(검수 재측정: 12k/24k/48k/96k 에서 대략 0.4/0.4/0.4/0.7초). "입력과 무관하다"는 뜻이 아니다.

**모델 자동 인지는 만들지 않는다.** 실측 근거: 환경변수 전수(`CLAUDE*` 17개, 검수 환경에서는 15개 — 환경마다 다르다)에 모델 성분이 없고, `CLAUDE_CODE_SESSION_ID` 로 세션 transcript 파일은 특정되지만 그 안의 모델 필드는 비공개 내부 포맷이며, 서브에이전트마다 모델이 달라 턴 단위 정확도가 성립하지 않는다. MCP `clientInfo` 는 클라이언트만 준다. 모델→프로파일 매핑을 갖는 순간 `role-catalog.js` 의 `claude-opus-4-7 "최신"` 과 같은 운명이고, 1.36.89 의 "모델 이름은 한 줄도 하드코딩하지 않는다"와 충돌한다. **토큰 절감량도 주장하지 않는다** — leerness 에 토큰 계측 코드가 0줄이다(`input_tokens|totalTokens` 검색 결과 0건). 문자수 감소만 실측으로 적는다.

**마지막으로 e2e 자신이 "삭제 안 됨"만 보고 "적용은 됐나"를 안 봤다.** 검수가 살려낸 변이 — 새 템플릿을 쓰지 않고 **이전 파일 뒤에 `Preserved` 헤더만 덧붙이는** 머지 — 가 무삭제 단언 3종을 전부 통과했다. 그러면 기존 설치본은 낡은 이력을 계속 **활성 본문**으로 읽는데 CI 는 초록이다. 부작용(삭제)은 봤지만 목적(교체)은 안 본 단언이었다. 관리 영역이 새 템플릿과 일치하고 옛 줄은 태그 뒤에만 있는지 단언한다. 그 단언조차 처음엔 정상 코드에서 실패했다 — 태그 앞에 붙는 구분선 `---` 을 반영하지 않았고, **제안받은 코드도 실행하기 전엔 맞는지 모른다**는 걸 30초 만에 확인했다.

**이 라운드에서 배운 것.** 외부 검수가 11회 차단했고 매번 실제 결함이었다. 마지막 다섯 번은 전부 **내 직전 수정이 만든 새 결함**이었고, 매번 내 자체 e2e 는 통과했다 — 내가 만든 픽스처는 내가 상상한 시나리오만 담기 때문이다. 검수에 **실제 설치본 21개를 함께 넘기자** "재현 못 함 → 미확인"이던 항목들이 "네 수치가 틀렸다"로 바뀌었다(설치본 잔존 0 → 실제 2/21, 코퍼스 121개 → 실제로는 빈 슬롯 포함 122). 그리고 마지막에는 **검수에게 출하 여부를 직접 물었다** — 내 "이번엔 다 잡았다"가 아홉 번 틀린 뒤였기 때문이고, 답은 "삭제 기능을 빼고 나머지만 내보내라"였다.

- 검증: e2e **415/415** · 회귀 가드 **변이 22종 22/22 사살**(옛 이력 필터 부활 · 이력 줄 삭제 · 새 템플릿 미적용 · 한쪽 언어만 정리 · 대문자 `**99 Tools**` · handoff 헤더 절단/O(n²) 복귀/서로게이트 분할/빈 파일 `1/1`/정확 상한 오판/Task Log 59 · `--json` 방향 뒤집기·`totalChars` 위조·마커 예산 이탈). 설치본 21개 실측 유실 0 은 **검수가 독립 재현**했고, CHANGELOG 의 "실측:" 문장은 전수 검증에서 정정 없이 통과했다.

## 1.36.94 — 2026-07-30 — 제품이 안내한 탈출구가 막다른 길이었다 · 재등록이 지운 인증 차단

헌트에서 확인만 하고 미뤄둔 것 중 **사용자가 갇히는 것**과 **보호가 조용히 꺼지는 것** 두 건을 처리했다. 그리고 첫 수정이 틀려서 되돌린 과정을 함께 적는다 — 그게 이 릴리스에서 가장 중요한 부분이다.

**차단 메시지가 안내한 탈출구가 그 상태에서 반드시 실패했다.** `bugfix-receipts.json` 이 형상 무효가 되면(가장 현실적인 경로는 git 병합 충돌 — `.harness` 는 추적 대상이다) 완료 판정을 보류한다. 어느 task 에 probe 가 걸려 있었는지 알 수 없으니 보류 자체는 옳다. 문제는 그다음이었다: 화면은 `leerness bugfix drop <ID>` 를 탈출구로 제시했는데 **그 명령도 같은 사유로 거부**했다. 등록·해제·완료가 전부 잠긴 상태에서 남는 실행 가능한 선택지는 안내문의 나머지 절반, 즉 `toggle set bugfix-receipt off` — **보호를 통째로 끄는 것** 하나였다.

**첫 수정은 `bugfix drop --force` 였고, 출하 전에 걷어냈다.** 도구가 손상 파일을 `.corrupt-<시각>` 으로 옮겨 주는 편의 명령이었는데, 검수에서 그 **한 명령이 데이터 유실 경로 다섯 개를 들여온 것**이 실행으로 확인됐다. ① 락 밖에서 `load → rename` 을 해, 검사 직후 다른 프로세스가 정상적으로 쓴 스토어를 "손상 원본"이라며 치웠다 — 대조 실측에서 정상 `drop` 은 락을 1회 잡는데 `--force` 는 0회였고, 두 프로세스 자연 경합 12초에서 옮겨진 파일 2851건 중 **556건(19.5%)이 유효한 receipt** 였다. ② `_loadChecked` 가 파싱 오류와 읽기 실패(EACCES/EIO)를 같은 `invalid` 로 뭉개는데 `--force` 는 원인을 구분하지 않아, 권한만 막힌 **멀쩡한** 스토어를 치웠다(Windows ACL 로 읽기만 막고 실측). ③ 형상 무효 분기에서 `<ID>` 를 아예 쓰지 않아 `drop T-9999 --force` 같은 오타나 ID 생략도 전체를 날렸다(대조: 정상 스토어에서는 같은 명령줄이 `probe 없음` 으로 거부된다). ④ 그 뒤 게이트가 무장해제되는데 성공 메시지의 "필요하면 `bugfix start` 로 다시 등록하세요"는 **거짓**이다 — 등록은 probe 가 지금 실패할 것을 요구하므로 버그를 고친 뒤에는 재등록이 거부된다. ⑤ 안내가 `--path` 를 잃어, `--path B` 로 막힌 사용자가 문구를 그대로 복사하면 **cwd 인 프로젝트 A** 가 파괴됐다.

되돌린 근거는 실측이다 — **파일을 손으로 고치거나 옮기는 회복은 그 다섯 가지가 전부 없고, 되돌리면 게이트도 되살아난다.** 1.36.93 의 결함은 "탈출구가 없었던 것"이 아니라 **"탈출구를 말해주지 않은 것"** 이었다. 그래서 이번 릴리스는 파괴적 명령을 추가하지 않고 안내만 고친다.

- 형상 무효를 거부하는 **모든 발화점**(`bugfix start` · `receipt` · `drop` · `list` · 완료 판정)이 `_recoverHint(root)` 로 같은 문장을 말한다 — 스토어 절대경로와, 옮기면 게이트가 무영향이 되고 되돌리면 되살아난다는 사실까지.
- 회복 절차가 **평문 · `task update --json` · `task sync` · `task sync --json` · `bugfix list --json` 다섯 표면 전부**에 도달한다. 종전엔 `escapeHint` 가 `store_invalid` 에서 `null` 을 돌려주고 `task sync` 가 `e.lines` 를 버려서, "틀린 안내"가 **"무안내"로 회귀**했다(검수에서 실측으로 잡혔다). 이제 다섯 표면이 같은 문자열을 쓰고 e2e 가 그 동일성과 exit 코드를 함께 단언한다.
- 안내 문구가 `--path` 를 유지한다. `BugfixBlocked` 가 `root` 를 함께 싣고 `_pathArg(root)` 가 cwd 와 다를 때만 붙인다.
- 실측: 손상 → `done` 보류(exit 1) → `drop --force` 도 **거부**(파일 무변경 · `.corrupt-*` 미생성) → 파일을 옮기면 `done` 성공 → 되돌리면 다시 보류. cwd=A 에서 `--path B` 로 막으면 안내가 B 의 경로를 가리킨다.

**`leerness provider add codex --bin codex` 한 번이 고위험 인증 차단을 껐다.** 사용자가 보기엔 아무것도 바꾸지 않는 재등록인데(실제로 바뀌는 것은 `source: builtin→user` 와 `desc` 뿐이다), provider 정규화가 6개 필드만 복사하면서 빌트인 `authCheck` 를 **조용히 버렸다**. 그 순간부터 그 프로젝트에서 codex 의 인증 축은 `unknown` 이 되고, "인증되지 않은 provider 로 고위험 작업을 확정할 수 없다"는 거부가 발동하지 않는다. 게다가 `agents check` 는 그 상태를 "등록된 확인 명령 없음"이라고 **단언**했다 — 등록돼 있는데 override 가 떨어뜨린 것이므로 사실이 아니다.

- 실측(판별 케이스: 검수 독립성을 충족시켜 인증이 *유일한* 차단 사유가 되게 구성): 로그아웃 상태 codex 로 고위험 작업이 **exit 0 · `confirmed: true`** 로 확정됐다. 수정 후에는 `provider_not_authenticated` 로 거부(exit 1)한다.
- **같은 실행파일일 때만** 물려받는다. 다른 실행파일에 남의 하위명령을 쏘지 않기 위해서다. 처음엔 문자열 완전일치로 비교했는데, 그러면 `codex.cmd`·대문자 `CODEX`·절대경로 같은 **정상 등록만으로 이번에 되살린 차단이 다시 사라진다**(검수 실측: Windows 에서 `CODEX --version` 은 같은 실행파일을 부른다). 확장자·대소문자·디렉토리를 정규화한 basename 으로 비교하고, e2e 가 별칭 4종을 전부 돌린다. 물려받지 못한 경우 `authCheckDroppedReason` 으로 사유를 말하고, `agents check` 화면도 "확인 명령 없음"이 아니라 "override 된 bin 이 달라 물려받지 않았습니다"로 바뀐다(`agents route` 화면에는 아직 전달되지 않는다 — 다음 라운드).
- 되돌리는 방법은 있었다 — `leerness provider remove codex` 로 override 를 지우면 즉시 복구된다(실측). 진짜 문제는 영속성이 아니라 **무엇이 꺼졌는지 알려주는 신호가 없었다**는 것이다. (재등록이 바꾸는 것은 `source`·`desc` 외에 `installHint`·`installCmd` 도 있다 — "아무것도 바꾸지 않는다"는 사용자 체감이지 사실이 아니다.)
- **이 차단은 fail-closed 가 아니다.** `auth === 'no'` 일 때만 막고 타임아웃·런처 실패·문구 불일치는 `unknown` 으로 통과시킨다(멀쩡한 사용자를 부당하게 잠그지 않기 위한 의도). 빌트인 10종 중 확인 명령이 등록된 것은 codex 하나뿐이므로, 이번 수정이 되살린 것은 그 **유일한** 차단 경로다. `unknown` 도 막을지는 정책 변경이라 별도로 다룬다.
- **`providers.json` 은 실행 인자를 정하는 통로가 아니다** — 그런데 `authCheck` 한 키만 막고 통로를 닫았다고 적었던 것이 검수에서 반증됐다. 실제로는 두 곳이 더 열려 있었다. ① `versionArgs` 가 `shell:true` 로 무검증 실행됐다 — `agents list` 한 번으로 임의 명령과 셸 메타문자가 돌았다(마커 파일 생성 · exit 0 · 경고 0). ② **`bin` 자체가 명령 위치다** — `cmd /c … & rem` 같은 값이 그대로 실행됐다. 그리고 두 값 모두 `provider sync` 로 **원격 catalog 에서** 들어온다.
- 그래서 신뢰 경계를 **모양이 아니라 출처**로 잡았다. 한때 `versionArgs` 를 "플래그 형태만"으로 좁혔다가 되돌렸다 — `kubectl version --client` · `gh copilot --version` · `go version` 처럼 하위명령이 정상인 CLI 를 false-BLOCK 해서 **설치된 도구를 미설치로 보이게** 했기 때문이다(검수 실측). 최종 규율은 세 가지다: `bin` 은 실행파일 이름/경로여야 하고(공백·메타문자 불가, 아니면 아무것도 실행하지 않는다), 로컬 `versionArgs` 는 메타문자가 없어야 하며(두 번째 명령을 만들 수 없다), **원격 `provider sync` 는 `versionArgs` 를 아예 정하지 못한다**(`--version` 고정 · 명령줄 `bin` 은 등록 자체를 거부). 메타문자만 막는 것으로는 부족하다 — `npm exec --yes <패키지>` 처럼 메타문자 없이 임의 코드를 받아오는 조합이 있다. 무시한 값은 `agents list` **평문에도** `⚠` 로 고지한다(JSON 에만 넣으면 사람은 왜 미설치로 보이는지 모른다). 빌트인 10종 부작용 0 실측.

**이번 라운드에서 배운 것.** 세 가지가 한 릴리스 안에서 동시에 재발했고 셋 다 이미 이름을 붙여 둔 실수다. (1) 탈출구를 만들면서 그 탈출구를 **데이터 규율 밖에** 두었다 — 1.36.87 이 `_upsert/_remove` 를 락 안에 넣은 이유를 알면서도 새 `--force` 는 락도 재로드도 없이 `renameSync` 를 불렀다. (2) **한 키만 막고 목표를 달성했다고 선언**했다 — `authCheck` 는 막았지만 릴리스 노트는 "임의 명령 실행 통로"를 닫은 것처럼 적었고, 같은 파일의 `versionArgs` 는 열려 있었다. 보안 목표는 키 이름이 아니라 "이 파일에서 실행되는 모든 것"으로 정의했어야 했다. (3) **새 테스트가 새 결함과 같은 시야를 공유**했다 — 첫 변이 7/7 은 내가 고른 7종에 대한 참이었고, 적대적 변이 4종(셋업 실패 미단언 · 전체 출력 정규식 · 못 찾은 줄에 대한 부정식이 공허 참 · 갈래 하나만 시험)이 살아남았다. 그래서 이번 가드는 셋업 성공과 저장 결과를 단언하고, 사유를 **구조(`refusals[].code`)에서** 읽고, 라벨을 긍정으로 단언하고, 주입을 user·빌트인 두 갈래로 시험하고, 심을 판별형(모르는 하위명령엔 usage 오류)으로 바꿨다.

그렇게 고친 가드에 변이를 다시 걸었더니 **또 3종이 살아남았고, 원인이 셋 다 달랐다.** ① `store_invalid` 에서는 공통 푸터가 원래 `null` 이라, sync 가 그것을 버리는 변이가 **등가 변이**였다 — 테스트가 그 한 사유만 덮고 있었다. ② 회복 문장이 스토어 절대경로를 직접 담아서, `--path` 부착 로직을 통째로 무력화해도 통과했다 — `--path` 는 사실상 미검증이었다. ③ 셋 중 하나는 가드 약점이 아니라 **변이 자체의 결함**이었다(`sub === 'add'` 가 파일에 9곳이라 엉뚱한 명령을 건드렸다). ①②를 메우려고 "probe 가 아직 실패" 사유로 한 번 더 막고, 그 푸터가 대상 절대경로와 `--path` 를 달고 있는지 · 평문/`--json`/sync 세 표면이 **같은 문장**인지를 단언하는 케이스를 넣었다. 파일이 사라졌을 때 예외로 죽던 단언도 명시 단언으로 바꿨다 — 크래시는 검증한 척할 뿐 사유를 남기지 않는다. 최종 변이 **23종 중 22종 사살**(`--force` 부활 · 경로 제거 · sync/`--json`/`bugfix list --json` 회복 절차 폐기 · 문장 공유 깨기 · `--path` 무력화 · root→cwd 바꿔치기 · 상속 제거 · bin 무시 상속 · bin 별칭 정규화 제거 · 상속 args 오염 · 사유 라벨 제거 · 주입 신뢰 2갈래 · bin 형식 가드 제거 · 원격 sync 의 versionArgs/bin 수용 · 평문 고지 숨김 · 폴백이 남의 하위명령 발사 · 셋업 강제 실패 · 비-`store_invalid` 푸터 폐기). 남은 1종은 **등가 변이**다 — `_checkAgent` 안의 2차 `_argsSafe` 를 무력화해도 결과가 같은데, 호출부를 전수 확인하니 사용자 데이터는 `_allProviders`(단일 병목)를 통해서만 도달해 그 층이 설계상 도달 불가이기 때문이다. 방어를 한 겹 남겨 두되 **커버리지로는 세지 않는다**.

## 1.36.93 — 2026-07-30 — 검증기가 못 보던 완료 행 · MCP 에서 영영 완료 못 하던 사용자

앞선 헌트에서 확인만 하고 미뤄둔 16건 중 **사용자가 갇히거나 검증 없이 완료되는** 두 건을 처리했다.

**`plan add "<작업>" --status verified` 한 줄이 검증기가 보지 못하는 완료 행을 만들었다.** `task add` 는 저장 시 `completed`/`verified` 를 `done` 으로 정규화한다(1.11.3 이 "집계마다 다르게 세는" 불일치를 없애려고 넣은 것). `plan add` 만 값을 검증만 하고 **원문 그대로** 저장했다. 결과는 같은 실행 안에서의 모순이었다 — `lazy detect` 는 그 행을 done 으로 세는데 `verify-claim --all` 과 `gate --claims` 는 `done` 필터에 걸러 **아예 보지 않았다**. `--force` 도 손편집도 필요 없이, 게이트를 통과했다는 신호가 검증되지 않은 주장을 덮었다.

- 같은 정규화를 태웠다. 실측: 정규화 안 된 행 0건 · `verify-claim --all` 대상 3 = done 행 수 3(이전 2 vs 3).
- **정규화는 plan.md 본문의 `Status:` 줄에도 적용된다** — 이건 부수효과가 아니라 함께 고쳐야 할 것이었다. 로드맵 상태 라벨 목록(`ROADMAP_STATUS_LABEL`)에 `verified` 키가 **없어서**, 종전에는 렌더러가 모르는 값이 plan.md 에 적히고 있었다. 덧붙여 상태 이름으로 "verified" 를 스스로 적는 것 자체가 이 제품이 막으려는 **자기선언 검증**이다 — 검증 여부는 검증기가 판정한다.
- 정규화 클래스를 전수 스윕했다: progress 행에 status 를 쓰는 경로 6곳(task add · plan add · task update · task sync ×2 · 초기 planned) 모두 정규화 또는 안전한 리터럴임을 실행으로 확인.

**MCP 표면에서는 정직하게 고친 사용자가 완료를 영영 기록할 수 없었다.** bugfix probe 예산은 600초인데(그래야 `npm test` 같은 현실적인 probe 를 등록할 수 있다) MCP 래퍼는 60초에 자식을 죽였다. 게다가 stdout 이 비어 응답이 **`(no output)`** 뿐 — 차단 사유도, 타임아웃이라는 사실도 전달되지 않아 진단 단서가 0 이었고, 재시도할 때마다 테스트가 60초 돌다 죽어 비용만 쌓였다. 하네스가 AI 에게 권하는 1차 표면이 MCP 라 더 나빴다.

- 기본 한도를 **660초**(probe 예산 위)로 올리고 `LEERNESS_MCP_TIMEOUT_MS`(5_000~3_600_000)로 조절 가능하게 했다.
- 타임아웃을 **사실대로** 보고한다 — 무엇이 중단됐는지 · probe 가 최대 600초 걸릴 수 있다는 사실 · 한도 조절 방법 · **동일한 CLI 명령**. MCP 클라이언트 자체의 타임아웃은 leerness 가 어쩌지 못하므로, 해결할 수 없는 부분을 해결한 척하지 않고 대안을 준다.
- 실측: 한도 5초 + `leerness_selftest`(~20초) → `(no output)` 대신 사유 전달 · 기본 한도에서는 정상 완료(20.8초).

**codex 33차 검수가 내 수정이 만든 새 결함을 잡았다(5건 전부 반영):**

- **한도를 전역으로 올린 것이 서버를 얼렸다** — `spawnSync` 는 단일 스레드 MCP 서버를 그 시간만큼 통째로 막는다. 무관한 도구가 멎으면 11분간 ping·취소 통지조차 못 읽고, 클라이언트가 자체 타임아웃으로 먼저 포기하면 사용자는 안내를 보지도 못한 채 서버만 9분 더 죽어 있다. → 긴 예산은 **probe 가 실제로 도는 호출에만**(`task update --status done|completed|verified` · `task sync`), 나머지는 60초. 함수 단위 6케이스로 확인.
- **타임아웃이 아닌 실패를 타임아웃으로 오인했다** — `status == null && !stdout` 은 EPERM·시그널 종료도 같은 모양이라, 권한 문제로 실패한 호출에 "한도를 늘리세요"라는 엉뚱한 처방을 냈다. **`ETIMEDOUT` 만** 타임아웃으로 보고, 나머지는 실제 오류 코드·시그널을 알리며, **중단 전 부분 출력을 버리지 않는다**.
- probe 설명을 **probe 가 돌 수 있는 호출에만** 붙인다(`leerness_selftest` 타임아웃에 bugfix 원인을 대지 않는다). 안내하는 CLI 명령은 공백 포함 인자를 인용한다.
- **내가 만든 시한폭탄 대체가 또 다른 시한폭탄이었다** — `patch − 3` 은 `1.37.0`·prerelease·patch<3 에서 0건을 돌려주고 `shown === totalVersions === 0` 이라 **공허하게 통과**한다. CHANGELOG 에 실재하는 버전을 뽑아 쓰고 `shown > 0` 을 함께 단언한다.
- 타임아웃 단언이 **소스 리터럴만** 봐서 `Math.min(...)` 같은 변조가 통과했다 → 메시지에 찍히는 초 값이 **실제 적용된 한도**와 일치하는지(5초 지정 → "5초")까지 확인하고, 배선(`timeout: budget` · `_needsProbeBudget`)도 함께 단언한다.

**시한폭탄 테스트도 하나 걷어냈다.** e2e 가 `whats-new --from 1.36.60` 이 "절단되지 않는다"고 단언했는데, 릴리스가 쌓여 그 창이 기본 상한 30 을 넘겼다(실측 31개 → truncated=true). 제품은 정상이고 **테스트가 스스로 만료**된 것이다. 기준점을 현재 버전에서 3단계 거슬러 자동 산출하게 바꿔, 릴리스가 아무리 쌓여도 창이 유지된다.

게이트: exit 0 · E2E 413/413 · selftest 341/341 · ✗ 0건

## 1.36.92 — 2026-07-30 — 동의 없는 외부 CLI 실행 차단 · 과소평가 우회 3종 · 손상 토글 fail-closed

1.36.88~91 에서 새로 만든 표면(bugfix 게이트·라우팅·대시보드·인증 축)을 6렌즈 적대 헌트(41 에이전트)로 훑어 **21건 확인 / 14건 반증**. 이번 릴리스는 그중 상위 5건 + codex 32차 검수 8건을 반영했고, 나머지 16건은 다음 라운드로 기록해 두었다.

**동의 없이 외부 CLI 를 실행하고 있었다(가장 심각).** `agents route "<작업>"` — 토글 OFF · `--confirm` 없음 · `LEERNESS_ENABLE_CODEX` 미설정 — 이 **제안 한 번**에 codex 를 두 번(`--version`, `login status`) 띄웠다. PATH shim 으로 argv 를 로깅해 증명했고, `LEERNESS_ENABLE_CODEX=1` 대조군이 **동일한 spawn** 을 내 문서화된 opt-out 이 이 경로에서 완전한 no-op 임을 확정했다. 공개된 capability 표면과 SECURITY.md 는 "사용자 동의 없이 외부 LLM/API/CLI 를 자동 호출하지 않는다"고 말한다. 원인: 인증 확인이 `plan()` 안에서 돌았고 `plan()` 은 토글·`--confirm`·거부 판정보다 **먼저** 실행된다.
→ 확인은 **확정 요청 + 토글 ON + provider opt-in(`LEERNESS_ENABLE_*=1`)** 이 모두 참일 때만. 제안 단계는 프로세스 0회.

**과소평가 우회 3종** — tiny 는 "범위가 좁다"는 주장인데, 규칙이 읽지 못하는 입력에서 그 주장이 통과했다:
- **다국어**: 고위험 규칙 어휘는 영어·한국어뿐인데 tiny 어휘(css/lint/typo)는 라틴이라 살아남아, `支付退款逻辑修改 css`(중국어 결제 환불)가 **tiny** 로 확정됐다.
- **전각**: `ＰＡＹＭＥＮＴ ｒｅｆｕｎｄ` 이 어떤 규칙에도 안 걸려 normal 로 떨어졌다(반각은 high-risk) → 매칭 전 NFKC 정규화(실측 3500자 200회 4ms).
- **제로폭**: `pa<ZWSP>yment re<ZWSP>fund css` 로 영문 키워드를 쪼개 우회 → 서식문자 제거.
- 악센트 라틴(스페인어 등)도 미커버로 본다. **한계는 그대로 적었다**: 악센트 없는 비영어 라틴 문장은 여전히 구분하지 못한다.

**손상 토글에서 게이트가 무언 통과했다** — `bugfix-receipt` 는 defaultOff 라 손상 = OFF 다. 앞선 수정은 `toggle list` 에 경고만 달고 **강제 시점을 그대로 뒀다**(codex 32차 #1 BLOCK: "화면에 경고를 띄우는 것과 실제로 막는 것은 다른 일"). 이제 done 전이에서 fail-closed 로 보류하고, `{"bugfix-receipt":"banana"}` 처럼 **JSON 은 멀쩡한데 값을 못 읽는** 경우도 같은 취급을 한다.

**그 밖에:**
- `bugfix start|receipt|drop` 이 위치 경로를 **조용히 무시**했다(`list` 만 인식) — 지목한 프로젝트는 무보호로 done 되고 cwd 의 같은 번호 task 가 갇혔다. 경로형 토큰을 인식하고(UNC 포함), 위치 경로와 `--path` 를 **둘 다** 주면 거부한다.
- 시크릿 마스킹: `Authorization: Bearer <토큰>` 이 통과하면서 옆에 `***` 가 찍혀 **가려졌다는 거짓 안심**을 줬다(안 가리는 것보다 나쁘다). 짧은 토큰(`Basic YTpi`)과 `~` 포함 token68 을 잡으려 규칙을 넓히면 산문(`Basic authentication flow`)이 훼손되므로, **authorization 헤더 문맥에 앵커**해 값 전체를 지운다 — 우회 7종 차단 · 오탐 0건 실측.
- 감사 기록은 **입력 원문**을 남긴다(분류용 정규화본이 아니라). 단 git 추적 파일이므로 시크릿 값만 가린다.
- 인증 미확인 사유를 구분한다 — "이 등급에서는 확인하지 않음" 과 "확인 수단이 등록되지 않음" 은 다른 말인데, 유일하게 확인 명령이 있는 codex 를 "수단 없는 provider" 로 읽게 만들고 있었다.
- `_EXEC_NOTE` 의 "상태 명령만 실행" 도 부정확했다 — `--version` 도 함께 돈다. 실행 시점과 대상을 정확히 적었다.
- `--approved-by` 의 제로폭 문자 제거 — 보이지 않는 승인자가 감사 기록에 남지 않게.

**가드**: PATH shim 으로 "프로세스를 띄우지 않는다"를 증명한다 — 종전 테스트에는 그걸 증명할 수단이 **아예 없었다**. 제안 0회 / 확정 N회 / opt-out OFF 0회의 3방향 판별을 단언하고, shim 은 배치되는 provider 전부(codex·claude)에 건다. 라우팅 변이 14/14 · 인증축 변이 9/10(1건은 등가 변이, 대조 변이 M9·M10 로 확인).

**작업 중 내 대조군이 내 수정을 잡았다**: 정규식 **리터럴**에 백슬래시를 두 개 넣어 `/[\\u0370-\\u03ff]/` 가 되면서 문자 클래스가 `\`·`u`·숫자·`f` 를 뜻하게 됐고, `fix typo in css` 의 `f` 가 "그리스 문자"로 잡혀 tiny 가 normal 로 떨어졌다. 대조군을 함께 돌리지 않았다면 "다국어 방어 강화"라고 보고하면서 멀쩡한 영어 입력을 망가뜨린 채 출하했을 것이다.

게이트: exit 0 · E2E 412/412 · selftest 341/341 · ✗ 0건

## 1.36.91 — 2026-07-30 — 인증 축: **설치됨 ≠ 사용가능**을 고지에서 실제 판정으로

1.36.89 의 라우팅은 "설치·활성만 확인했고 인증·쿼터·과금 권한은 확인하지 않았다"를 **말하기만** 했다. 이번엔 확인할 수 있는 것을 실제로 확인하고, 확인할 수 없는 것은 그대로 남긴다.

**무엇을 확인할 수 있는지 먼저 실측했다.** 후보 CLI 11종의 `--help` 를 읽어 인증 관련 하위명령을 찾고, 확실히 무료인 것만 실행했다:

- `codex login status` → exit 0 · **158ms** · "Logged in using ChatGPT" → **등록**
- `claude auth` → status 하위명령 없음(exit 1) → unknown 유지
- `ollama list` → **8초 타임아웃, Ollama 앱을 기동시킨다**(부작용) → **등록 제외**
- 나머지 7종 → 무료·비대화형 확인 수단 없음 → unknown 유지

등록 기준을 코드에 못 박았다 — ① 모델을 호출하지 않아 과금이 없다 ② stdin 없이 끝난다 ③ 서비스/앱을 기동하지 않는다. 셋 다 만족하지 못하면 **추정하지 않고 unknown 으로 남긴다**. 지금 10종 중 1종만 판정되는 것이 정직한 현실이다.

- **`agents check`** 가 인증 축을 별도로 보고한다(`auth: ok | no | unknown` · `authVerified` · `authUnknown`). `status`(설치+활성)의 의미는 그대로 두었다 — 두 검증을 한 단어에 섞으면 무엇이 확인됐는지 알 수 없어진다.
- **`agents list` 는 확인하지 않는다** — 목록 조회마다 외부 프로세스를 띄우면 안 된다.
- **라우팅 연동의 비대칭**: `unknown` 은 막지 않고 **확실한 `no` 만** 고위험에서 차단한다. unknown 을 차단 사유로 쓰면 확인 수단이 없는 대부분의 CLI 에서 고위험 작업이 전부 잠긴다(false-BLOCK).

**codex 30차 검수(BLOCK 1 포함 7건)를 전부 반영했다:**

- **라우팅이 다른 실행파일의 인증을 보고할 수 있었다** — `EXTERNAL_AGENTS` 를 직접 봐서 프로젝트의 provider 재정의(`_allProviders`)를 무시했다. `agents check` 는 재정의된 CLI 를, 라우팅은 전역 동명 CLI 를 확인하는 불일치. 같은 해석기를 쓰도록 고쳤다.
- **모든 nonzero 를 "확실히 미인증"으로 읽었다** — 구버전(하위명령 없음)·설정 오류·내부 크래시도 nonzero 다. `no` 는 고위험을 차단하므로 이건 멀쩡한 사용자를 잠근다. 이제 **로그아웃으로 알아볼 수 있는 문구**(`noPattern`)가 함께 잡힐 때만 `no` 이고, 아니면 unknown 이다. ⚠ 로그아웃 상태의 실제 codex 출력은 관측하지 못했다(로그인 상태였다) — 패턴이 빗나가면 unknown 이 되어 **안전한 방향으로** 실패한다.
- **라우팅이 "외부 CLI 를 실행하지 않는다"고 했는데 거짓이 됐다** — 인증 확인이 실제로 CLI 를 실행한다. 문구를 사실에 맞추고(모델 호출은 하지 않음 · 고위험에서 상태 명령만 실행), 확인 자체를 **고위험 등급에서만** 하도록 좁혔다(tiny/normal 은 지연도 실행도 없다).
- **evidence 가 하위 프로세스 출력을 그대로 재발행했다** — 토큰 마스킹 + 제어문자(터미널 이스케이프) 제거. 마스킹 구현은 `lib/pure-utils.js` 로 올려 대시보드와 **같은 함수**를 쓴다(표면마다 다시 구현하면 한쪽만 고쳐진다).
- 셸 메타문자가 든 `authCheck.args` 는 실행 자체를 거부한다(인용이 깨져 엉뚱한 실행이 되고 주입 표면이 된다).
- e2e 의 "확인 명령 없는 provider 는 unknown" 단언이 **provider 누락도 성공으로 읽었다**(`!a || …`) → 존재를 먼저 요구.
- "무료·비대화형 확인 명령 없음"은 과장이었다(없다고 증명한 게 아니라 **등록된 것이 없을** 뿐) → 문구 정정.

**가드**: 변이 9종 중 8종 검출 + 1종은 **등가 변이**로 확인했다(정상 등급에서는 인증을 계산하지 않으므로 차단 조건을 바꿔도 동작 차이가 없다 — 같은 지점을 실제로 깨는 대조 변이 M9 는 잡힌다). "인증 확인을 통째로 끄는" 변이가 처음에 살아남았는데, 확인이 **실제로 수행되는지**를 아무도 단언하지 않았기 때문이다 — 종료코드를 테스트가 정하는 합성 provider 로 메커니즘 자체를 검사하게 고쳤다(실제 CLI 설치 여부에 의존하는 단언은 그 CLI 가 없는 머신에서 공허해진다).

**작업 중 자체 가드가 나를 두 번 잡았다**: ① 제어문자 제거용 정규식에 **생 NUL 을 그대로 넣어** 1.36.81 의 "출하 소스 생 NUL 금지" 메타가드가 selftest 를 실패시켰다 ② 1.36.89 의 route 가드가 옛 정직성 문구를 단언하고 있어, 문구가 거짓이 되어 고치자 실패했다 — 가드를 새 주장에 맞추면서 **옛 과장 문구가 되살아나지 않는지**도 함께 단언하도록 보강했다.

게이트: exit 0 · E2E 411/411 · selftest 341/341 · ✗ 0건
...
- **High — 플래그십 게이트가 가장 위험한 케이스에서 반대로 작동했다.** `verify-claim` 은 evidence 에 파일 경로가 없으면 `fileChecks.every([])`=true(공허참) · `stubFiles`=[] 이라 구현실체 true · git 교차검증은 `files.length>0` 을 요구해 skip — **모든 판정이 공허참**이 되어 "코드도 테스트도 없이 done" 이 `ok:true`/exit 0 이었다. **단조성이 뒤집혀 있었다**: 파일명을 적어 검증 가능하게 쓸수록 FAIL, 아무것도 안 적고 날조할수록 PASS. 코드 주석은 "공허참은 evidenceQualityOk 가 차단한다"고 적혀 있었지만 기본 모드의 그 값은 `hasFile||hasTest||hasLog` 라 `"tests: 32/32 passed"` 문자열 하나로 통과했다 — **주석의 전제가 거짓**이었다. 새 사유 `unverifiable-claim` 을 세 경로(collect/`--json`/human)가 공유하는 한 곳에 배선. 탈출구는 두 가지뿐(evidence 에 수정 파일 경로를 적거나 `--run-tests` 로 실행 결과를 붙이거나), 기존 `--lenient` opt-out 은 그대로.
  - **한계(과장하지 않음)**: 이 수정은 "대조된 것이 하나도 없는" 구멍을 닫을 뿐, 존재하는 아무 파일명(예: `README.md`)을 적으면 통과한다. verify-claim 은 증거 대조이지 구현 증명이 아니다.
  - **추출기 사각지대 동반 수정**: "대조 0건 = FAIL" 로 바꾸면 추출기가 못 보는 파일이 곧 정직한 주장의 거짓 차단이 된다. 확장자 없는 표준 파일명(`Dockerfile`/`Makefile`/`LICENSE` 등)을 추출 대상에 추가하고, `_evidenceQuality` 의 "파일 언급" 판정도 같이 맞췄다(주석이 "bin FILE_RE 와 정합"을 명시하는데 한쪽만 고치면 같은 명령 안에서 정의가 둘로 갈린다).
...
- **문서 수치 메타가드**: `docs/interoperability.md` 가 "86 도구"(3릴리스 낡음)를, `README.ko.md` 가 "selftest 210 케이스"(실제 336)를 **현재 상태**로 주장한 채 npm 에 실려 나갔다. `README.md` 는 `readmeSync` 관리구간이라 동적으로 맞았지만 관리구간 밖 문서는 아무도 맞춰주지 않는다. 낡을 수치를 살아있는 출처 안내로 대체하고, README/docs 의 MCP 도구·selftest 수가 실측과 어긋나면 selftest 가 실패하도록 했다(과거 라운드 로그인 AGENTS/CLAUDE/CHANGELOG 는 과거 수치가 정당하므로 제외 — 포함하면 false-BLOCK).
- **가드를 만들며 두 번 틀렸고 둘 다 실행으로 잡았다**: (1) 사이트 수치 가드 1차판이 5/5 오탐(주석·파생표현·다른 개념의 "0 의존성")이라 대상을 실제로 낡은 이력이 있는 수치로 좁혔다. (2) 그 가드가 하드코딩 재삽입을 **차단하지 못했다** — JS 의 `\b` 는 ASCII 기준이라 `(?:도구|tools?)\b` 가 한글 뒤에서 성립하지 않았다. 차별적 통제를 돌리지 않았다면 아무것도 지키지 않는 가드를 출하할 뻔했다. 같은 버그로 내 수동 grep 도 낡은 주장 2건 중 1건만 보고 있었다.
**검수 24회전이 BLOCK 판정 + High 6건** — 전건 직접 재현 후 수정. 이번 라운드는 **내 자체 검증이 여섯 개를 다 놓쳤다**:

- **내가 고치려던 역전을 다시 만들었다.** `_runProven` 을 `runResult.parsed` 로 판정한 탓에, `--test-cmd 'node -e "process.exit(0)"'` 처럼 **실제로 실행해 exit 0 을 받아도** 출력이 파싱 불가면 차단되고, evidence 에 `"1/1 passed"` 문자열만 적은 날조는 통과했다. 커스텀 리포터·`go test` 를 쓰는 정직한 실행이 벌을 받는 구조 — 진단 신호로 삼았던 단조성 역전 그 자체다. 판정 기준을 **실행 확증**(`allPassed`)으로 교체.
- **추출기 사각지대 4형태가 곧 거짓 차단이었다**: `ops/containers/Dockerfile`(다단계 경로의 bare 파일) · `.github/workflows/ci.yml`(선두 점이 잘려 `github/…` 라는 없는 파일로 판정) · `.gitignore` · `Dockerfile.dev`(내가 추가한 bare 규칙이 `Dockerfile` 로 잘라 "없는 파일"로 만듦). bin 추출기와 `_evidenceQuality` 로 갈려 있던 판정을 **단일 출처**(`lib/analyzers._EVIDENCE_FILE_RE`)로 통합.
- **툴이 안내하는 문법 그대로 쓰면 깨졌다.** `--require-referee` 를 value-flag 목록에 등록하지 않아 값이 positional 경로로 흡수돼 root 가 `<cwd>/<id>` 가 됐다(소스 주석이 "신규 value-flag 는 반드시 여기 등록" 이라고 명시하는데 어겼다). **내 e2e 는 항상 `--path` 와 `.` 을 줘서 이 결함을 가리고 있었다** — 픽스처가 버그를 숨기는 전형이라, 이제 안내된 문법 그대로(cwd + 플래그만) 검증한다. 1.36.80/81 의 다른 신규 value-flag 도 소급 등록.
- **값 없는 `--require-referee` 가 조용히 사라졌다** — CI 의 `--require-referee=$EMPTY_ENV` 가 아무 게이팅 없이 exit 0(fail-open). 명시적 실패 단계(`referee_missing_value`)로 전환.
- **메타가드 자신이 반쯤 공허했다**: 케이스 파싱이 337 중 170 만 잡았고, 자기 제외 표식을 `'selfref' + '-guard-own'` 로 조립해 **소스에 결합된 문자열이 없어 한 번도 동작하지 않았다**(자기를 세지 않은 건 우연). 백틱 리터럴로 우회도 가능했다. 시작 경계 분할로 전수 파싱 + 백틱 포함 + 표식 교정, baseline 은 과소집계였던 9 → **실측 12** 로 정정.
- **"탐지력 증명됨" 은 실제보다 강한 주장이었다** — 캘리브레이션이 확인하는 것은 등록된 known-good/known-bad 쌍이 기대 사유로 갈린다는 것과 `check` 의 기동 가능성뿐이고, good/bad 가 `check` 와 같은 절차인지는 **도구가 강제할 수 없다**. 문구를 사실에 맞추고 그 조건을 명시.
- Medium 동반 수정: gate 의 referee 실패가 `--json` 에서 `{name, ok:false}` 로만 축약되던 것 → `code`/`exit`/`timedOut`/`spawnError`/`signal` 노출 · 캘리브레이션 timeout 정책을 gate 와 공유(느린 검증기 거짓 차단 방지) · 같은 id 반복은 순서 보존 dedupe · 문서 수치 가드의 어순 양방향·core 문맥·`80+` 하한 표현·`CONTRIBUTING/SECURITY` 스캔 보강.

- 검증: selftest 337 · e2e **406/406**(신규 2블록) · 신규 단언의 **변이 테스트**(수정을 되돌리면 정확히 실패) · 메타가드 4방향(따옴표/백틱 자기참조 차단 · 정상 가드 무오탐) · false-BLOCK 시나리오(문서만 수정 / 추출기 사각지대 5형태 / 비-done / `--run-tests` 실행확증 / `--lenient`) 실측.
...
- **F12**: skills-lock 은 "요청 집합"이 아니라 **디스크 실재**를 기록 — `--skills all` 후 `recommended` 재init 시 9개 디렉토리가 남는데 락은 5개만 주장하던 불일치를 합집합 재생성으로 해소(실측 dirs 9 = lock 9). 스킬 제거는 하지 않음(비파괴 원칙).
- **F9 (설계 결정)**: 초고령 KO canonical 이 en 프로젝트에 커스텀으로 오인 이월되는 것 — **자동 차감은 하지 않는다**(구판 템플릿 라인과 사용자 노트를 확신으로 구분 불가, false-DROP 이 버그·false-PRESERVE 는 무해). 대신 한글 비중 높은 이월(≥10줄·>50%) 시 Preserved 섹션에 **검토·정리 안내**를 명시해 조용한 동결을 없앤다. 안내는 멱등(재이월 안 됨), 소수 한글 커스텀·ko 프로젝트엔 무경고.
- 검증: F12 재현 시나리오 실측, F9 순수함수 5프로브(안내/멱등/무손실/소수 무경고/ko 무경고), selftest 334, e2e +1.
...
- **기본 최신 30개 버전 + 정직한 절단 표기**(`totalVersions`/`shown`/`truncated`/`hint`), 전체는 `--all`, 개수는 `--limit N`. 텍스트 모드 무변경. 실측 804KB → 30KB.
- (검수 #1 Medium) **MCP 계약 동반 갱신**: `leerness_whats_new` 에 `limit`/`all` 인자 노출 + 전달 — 종전 그대로면 MCP 소비자는 30개로 조용히 잘리고 조절 수단이 없었다(실측 확인 후 수정).
- (검수 #2) 무효/비양수 `--limit`(0/-5/abc) 는 기본값 폴백(종전 -5 → 1). (검수 #3) 절단 힌트 UI 언어 준수. (검수 #4) e2e 단언을 `shown === 30` 정확 일치로 + 무효 limit·비절단 hint 부재·MCP 전달 케이스 추가.
- **후속 계측(검수 제공)**: migrate stdout ~88KB · memory search(--limit huge) ~54KB · retro/insights ~40KB · task list ~33KB — 다음 상한 후보로 기록(현재는 안전 범위).
- 검증: selftest 334, e2e +1(크기 회귀 가드 포함).

## 1.36.68 — 2026-07-22 — F14 재구현: 마감 보고의 미승인 미리보기 (8차 헌트 이연분 종결) — R-0001 검수 11회전

1.36.67 에서 이연했던 F14 를 원인 규명 후 재구현.

- **이연 원인(규명)**: 이전 삽입 지점이 `try/finally` 밖 **jsonMode 전용 후처리 구간**이라 텍스트 경로가 도달조차 못 했다(추정 아님 — 코드 구조 확인). 텍스트 렌더 구간으로 이동해 양 모드 동일 데이터 출력.
- **`session close` 가 승인 대기 미리보기를 보고**: 텍스트 블록(최대 5줄) + `pendingPreviews {count, items[≤10]}` JSON 필드. proposed/revision-requested 는 대기, approved 는 제외. 손상 previews.json 은 count 0 으로 안전 강등(마감 중단 없음 — 검수 확인).
- (검수 Medium) **`--no-suggest`/`LEERNESS_NO_SUGGEST=1` 에서도 노출**: suggest 블록 안에 있어 승인 대기 상태가 사라지던 것 — 공용 구간으로 이동 + e2e 회귀 가드.
- 검증: 텍스트/JSON 일치·JSON 순수성·12건 캡(텍스트 5·items 10)·en 렌더·승인 시 소거(검수 프로브), selftest 334, e2e +1.

## 1.36.67 — 2026-07-22 — tech 갱신 시 그래프 동반 정합 (8차 헌트 F15)

- **F15**: `leerness tech` 가 프로필 변경을 감지하면 이미 존재하는 `leerness.html` 의 🛠 탭도 함께 갱신 — 종전엔 프로필만 최신이고 그래프는 옛 서비스를 계속 보여줬다(다음 handoff 전까지 불일치). 그래프 산출물이 없는 프로젝트는 무동작(생성 강제 안 함).
- **정직 이연 — F14(마감 보고의 미승인 미리보기)**: JSON 필드는 정상 동작했으나 텍스트 경로에서 출력 위치/스트림 상태가 맞지 않아 **반쪽 상태로 출하하지 않고 되돌림**. handoff 헤드라인 노출은 기존대로 유지되므로 기능 손실 없음. 다음 라운드에서 close 렌더 파이프라인 정비와 함께 재구현.
- 검증: 변경→그래프 동기화/무-그래프 프로젝트 무동작 실측, selftest 334.
...
- **무명 확장자 소형 텍스트 스캔**: `config.custom`/확장자 없는 credential 파일 미탐 해소 — ≤64KB + NUL 스니핑 + 이진 확장자 deny(검수 #4: .bin/.dat 등 추가). (검수 #2) 확장자 없는 파일('')이 기존 무제한 경로로 새던 것을 가드 경로로 통일 — 대형/이진 무명 파일도 이제 안전장치 적용.
- **중복 보고 dedupe**: (검수 #1) 시작-오프셋 키는 실제 중복(컨텍스트 패턴=키 시작 vs 직접 패턴=값 시작)을 못 접었다 — **값 스팬(시작+길이) 키**로 보정. 실측: `api_key = "npm_…"` 2건→1건, 서로 다른 토큰 2건 유지.
- **성능**: (검수 #3) 파일당 stat 2회+.gitignore 재읽기(5천 파일 실측 5천회) — stat 1회 + .gitignore 수집당 1회 캐시.
- 검증: selftest 329(+1: 동일토큰 1건/통제군/무확장 64KB·NUL 가드), 게이트, R-0001 검수 사이클 1회전 완료.
...
- **#1 High — preview 형상-무효 스토어 파괴/크래시**: 파싱되지만 무효(비배열 루트/null 항목)를 []로 오인해 덮어씀(6차 rules 와 동일 클래스 재발!) — 로더 형상검증 + 변경 진입점 store_invalid 거부.
- **#2 High — 동시 preview add lost-update**: 락 없는 read-modify-write — _withLock 직렬화(락 안 재로드). 실측: 동시 2건 모두 생존·고유 ID.
- **#3 — clarify FP/FN**: "A 또는 B 모두 테스트" 오질문(모두/둘다/both 억제) + "좀 더 좋게" 미감지(비교급 모호 패턴 추가).
- **#4 — DATABASE_URL→postgres 오단정**: DB 중립 키 제외(.env 값 안 읽는 원칙 유지). **#5 — 모노레포 하위 매니페스트 미감지**: 2단계 병합 스캔. **#6 — 스키마-무효 프로필 고착**: {} 를 이전 프로필로 오인해 영구 미갱신 — 부재 취급 재생성. **#12 — requirements 점-접두 오탐**(openai.fake→openai): 배포명 전체 일치.
- **#7 — io 계약 모순 2종**: failJson 후 log(JSON) 이중 문서(래치 후 JSON 통과는 지연-fail 전용) + fail 후 회복 exit 0 인데 폴백 오류 JSON(최종 exit≠0 일 때만 방출). **#8 — 같은 ms 백업명 충돌**: pid+시퀀스 접미.
- **#9 — 동등 주장의 표현별 판정 불일치**: 실행-우선(1.36.52)을 비율형만 적용해 "3 tests passed"와 "3/3 passed"가 갈렸다 — 카운트형에도 동일 적용(실행 미확증 정적 게이트는 유지).
- **#10 — approve 중복 이력/빈 revise**: 멱등 no-op + note_required. **#11 — 제목 개행**: 한 줄 정규화.
- 검증: 12/12 행위 재검 실측(동시성·형상·io 3종·양방향 verify-claim 포함), selftest 328(+1 통합), e2e.
...
- **e2e 단언 4건 현행화**: 자동생성 대상 단언을 leerness.html 로 (수동 `leerness roadmap` 의 roadmap.html 테스트 3건은 불변 — 여전히 유효한 기능).
- **게이트 절차 교정**: 러너에서 exit 마스킹 echo 제거 — npm test 의 exit 가 그대로 전파. 성공 판정에 "E2E result: N/N passed" 라인 존재를 필수 조건으로 추가.
- **검증**: 이번 릴리스는 전체 게이트(selftest 301 + e2e 전체)를 exit 전파형으로 통과한 것만 게시.
...
- **handoff `⚕ health` 토큰 → `🔐 .env`**: 이 헤드라인 토큰은 실제로 **.env 의 .gitignore 보호 여부만** 봤는데 "종합 health(보안+drift)"로 표기했다(주석까지 그렇게 거짓). 커밋된 시크릿/drift/상태무결성은 안 봄 → 라벨을 `🔐 .env` 로 정직화(종합 health 아님 명시).
- **policy enforce "초과 명령 차단"** → `초과 명령 = 차단 판정 + exit 1 신호 · 실제 셸 실행을 가로채진 않음`: leerness 는 셸을 감싸지 않으므로 enforce ON 이어도 협조적 verdict/exit code 일 뿐, 하드 실행 차단이 아님을 명시.
- **KO identity/about "허위 완료 차단" → "허위 완료 감지 (권고; CI 게이트 필수화 시 차단)"**: EN 설명은 이미 협조적(권고)임을 밝히는데 KO 정체성/about/README 는 하드 "차단"으로 과장 → EN 과 일치시킴(사용자 프로젝트 README 에 기록되는 고가시성 문구).
- **README "measures the real count and rejects a mismatch"** → 기본은 **테스트 선언(정적 카운트)** 이 주장 초과 시 거부하는 것이지 "통과" 증명이 아님을 명시(+`--run-tests` 로 실제 실행 검증).
- selftest 280 (신규 정직성 후속 소스가드, split-literal self-ref 회피) + e2e 통과.
