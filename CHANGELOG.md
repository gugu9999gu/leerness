...
- Windows/Node 18의 deletion-pending 상태가 빈 `.lock`을 남기던 문제를 토큰 고유 release tombstone으로 해결했습니다. 해제 재시도는 새 경쟁자 락을 건드리지 않으며, native Promise·비표준 thenable·ABA·재시도 소진 경로를 결정적 회귀로 검증합니다.
